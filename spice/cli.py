...
def main_event_inference(args):
    ...
    if invalid_steps:
        raise ValueError(
            f"Invalid step(s): {', '.join(invalid_steps)}. Valid steps are: preprocessing, split, all_solutions, disambiguate, large_chroms, combine"
        )
    ...
    if args.unlock:
        ...
        if not os.path.exists(snakefile):
            raise FileNotFoundError(
                f"Snakefile_event_inference not found at {snakefile}"
            )
    ...
    if args.snakemake:
        ...
        if args.snakemake_mode == "slurm":
            ...
            cmd.extend(["--cores", str(args.snakemake_cores)])
    ...
    if args.clean:
        ...
        if not name:
            raise ValueError("Config file must specify a 'name' field.")
        results_events_dir = os.path.join(
            config["directories"]["results_dir"], name, "events"
        )
    ...
    from spice.event_inference.pipeline import (
        full_paths_from_graph_with_sv_wrapper,
        solve_with_knn_wrapper,
        solve_with_mcmc_wrapper,
    ...
    if args.debug and args.cores and args.cores > 1:
        logger.warning(
            "Debug mode with multiple cores may lead to interleaved log messages."
        )
    ...
    logger.info(
        f"Running event inference for project name {name} with config file {args.config_path}"
    )
    ...
    if n_samples > 50:
        ...
        logger.warning("")
        logger.warning(
            "SPICE can be very slow when processing many samples in serial mode."
        )
        logger.warning(
            "For large datasets, we strongly recommend using the Snakemake workflow"
        )
        logger.warning("for parallel execution on a cluster.")
        logger.warning("")
        logger.warning(
            "See README section 'Using with Snakemake' for more information:"
        )
        logger.warning("=" * 80)
    ...
    if not args.keep_old and args.ids is None:
        if config["params"].get("skip_existing", False):
            raise ValueError(
                "If skip_existing=True in config, have to use --keep-old to avoid deleting existing files."
            )
    ...
    if "preprocessing" in which and args.run_preprocessing:
        from spice.preprocessing.extra_preprocessing import (
            main as extra_preprocessing_main,
        )
        ...
        logger.info(
            "Skipping preprocessing (default). Use --run-preprocessing to enable it."
        )
    ...
    if "split" in which:
        ...
        split_tsv_file(
            name, keep_old=args.keep_old, cores=args.cores, selected_ids=selected_ids
        )
    ...
    if "all_solutions" in which:
        ...
        for wgd_status in ["nowgd", "wgd"]:
            ...
            cur_ids = [
                x.replace(".pickle", "")
                for x in os.listdir(
                    os.path.join(str(results_events_dir), wgd_status, "chrom_data_full")
                )
            ]
            if selected_ids is not None:
            ...
            def run_full_paths(cur_id):
                output_file = os.path.join(
                    results_events_dir,
                    wgd_status,
                    "full_paths_multiple_solutions",
                    f"{cur_id}.pickle",
                )
                if skip_existing and os.path.exists(output_file):
                    logger.info(
                        f"Skipping all_solutions for {cur_id} ({wgd_status}) as {output_file} exists."
                    )
                    return {
                        "status": "skipped",
                        "cur_id": cur_id,
            ...
            save_fail_reports(
                cur_failed_reports, cur_step=wgd_status + "_all_solutions"
            )
    ...
    if "disambiguate" in which:
        ...
        for wgd_status in ["nowgd", "wgd"]:
            if not os.path.exists(
                os.path.join(
                    str(results_events_dir), wgd_status, "full_paths_multiple_solutions"
                )
            ):
                logger.info(
                    f"Directory {os.path.join(str(results_events_dir), wgd_status, 'full_paths_multiple_solutions')} does not exist, skipping disambiguation for {wgd_status}"
                )
                continue
            is_wgd = wgd_status == "wgd"
            cur_ids = [
            ...
            def run_knn(cur_id):
                ...
                if skip_existing and os.path.exists(output_file):
                    logger.info(
                        f"Skipping disambiguate for {cur_id} ({wgd_status}) as {output_file} exists."
                    )
                    return {
                        "status": "skipped",
                        "cur_id": cur_id,
                ...
                return solve_with_knn_wrapper(
                    ...
                    single_width_bin=config["params"].get(
                        "knn_single_width_bin", False
                    ),
                )
            ...
            results = _run_batch(
                cur_ids,
                args.cores,
                f"Disambiguate solutions ({wgd_status})",
                run_knn,
                logger,
            )
            cur_failed_reports = [
    ...
    if "large_chroms" in which:
        ...
        stop_after_no_improvement = config["params"].get(
            "mcmc_stop_after_no_improvement", None
        )
        for wgd_status in ["nowgd", "wgd"]:
            if not os.path.exists(
                os.path.join(str(results_events_dir), wgd_status, "chrom_data_large")
            ):
                logger.info(
                    f"Directory {os.path.join(str(results_events_dir), wgd_status, 'chrom_data_large')} does not exist, skipping large chromosomes for {wgd_status}"
                )
                continue
            is_wgd = wgd_status == "wgd"
            cur_ids = [
                ...
                for x in os.listdir(
                    os.path.join(
                        str(results_events_dir), wgd_status, "chrom_data_large"
                    )
                )
            ]
            if selected_ids is not None:
            ...
            def run_mcmc(cur_id):
                output_file = os.path.join(
                    results_events_dir,
                    wgd_status,
                    "mcmc_solved_chroms_large",
                    f"{cur_id}.pickle",
                )
                if skip_existing and os.path.exists(output_file):
                    logger.info(
                        f"Skipping large_chroms for {cur_id} ({wgd_status}) as {output_file} exists."
                    )
                    return {
                        "status": "skipped",
                        "cur_id": cur_id,
            ...
            results = _run_batch(
                cur_ids,
                args.cores,
                f"Large chromosomes ({wgd_status})",
                run_mcmc,
                logger,
            )
            cur_failed_reports = [
    ...
    if "combine" in which:
        ...
        solved_dirs = (
            [
                os.path.join(results_events_dir, wgd, "knn_solved_chroms")
                for wgd in ["nowgd", "wgd"]
            ]
            + [
                os.path.join(results_events_dir, wgd, "full_paths_single_solution")
                for wgd in ["nowgd", "wgd"]
            ]
            + [
                os.path.join(results_events_dir, wgd, "mcmc_solved_chroms_full")
                for wgd in ["nowgd", "wgd"]
            ]
            + [
                os.path.join(results_events_dir, wgd, "mcmc_solved_chroms_large")
                for wgd in ["nowgd", "wgd"]
            ]
        )
        combine_final_events(
...
def main_plotting(args):
    ...
    if args.plot_events_per_sample is not None:
        ...
        chrom_segments = pd.read_csv(
            chrom_segments_file, sep="\t", index_col=["sample_id", "chrom", "allele"]
        ).sort_index()
        ...
        out_path = os.path.join(
            plots_base_dir,
            f"{cur_sample}_events{'_unit_size' if args.plot_unit_size else ''}.png",
        )
        ...
        output_dir = os.path.join(
            config["directories"]["results_dir"], name, "loci_of_selection"
        )

        logger.info(f"Plotting all loci on {cur_chrom} ({detection_assignment} mode)")

        data_per_ls = open_pickle(
            os.path.join(output_dir, "data_per_length_scale", f"{cur_chrom}.pickle")
        )
        selection_points = open_pickle(
            os.path.join(
                output_dir,
        ...
        out_path = os.path.join(
            plots_base_dir, f"{cur_chrom}_loci_{detection_assignment}.png"
        )
        fig.savefig(out_path, bbox_inches="tight")
        logger.info(f"Saved plot to {out_path}")
        ...
        final_loci_df = pd.read_csv(
            os.path.join(
                ...
                name,
                f"final_loci_{detection_assignment}.tsv",
            ),
            sep="\t",
            index_col=0,
        )
        ...
        if loci_index not in final_loci_df.index:
            raise ValueError(
                f"Locus index {loci_index} not found in final loci dataframe for {detection_assignment} mode. Available indices: {final_loci_df.index.tolist()}"
            )
        ...
        logger.info(
            f"Plotting locus index {loci_index} on {cur_chrom} ({detection_assignment} mode)"
        )
        ...
        out_path = os.path.join(
            plots_base_dir, f"{cur_chrom}_locus_{loci_index}_{detection_assignment}.png"
        )
        fig.savefig(out_path, bbox_inches="tight")
        logger.info(f"Saved plot to {out_path}")
...
def main_loci_detection(args):
    ...
    loci_results_dir = os.path.join(
        config["directories"]["results_dir"], config["name"], "loci_of_selection"
    )
    ...
    if args.snakemake:
        ...
        if not os.path.exists(snakefile):
            raise FileNotFoundError(
                f"Snakefile_loci_detection not found at {snakefile}"
            )
    ...
    for chrom in chromosomes:
        ...
        run_loci_detection_per_chrom(
            ...
            overwrite_preprocessing=(
                loci_params["overwrite_preprocessing"] and args.overwrite
            ),
            name=config["name"],
            ...
            optimizing_N_iterations_optimization=loci_params[
                "optimizing_N_iterations_optimization"
            ],
            ...
            final_limiting_N_iterations_optim=loci_params[
                "final_limiting_N_iterations_optim"
            ],
    ...
    if not (
        steps_to_run in ["fast", "default", "combine"]
        or "combine" in steps_to_run
        or "+" in steps_to_run
    ):
        logger.info(steps_to_run)
        logger.warning(
            "Loci detection steps do not include 'combine'. Final combination of loci across chromosomes will be skipped."
        )
        return
    ...
    final_loci_output_path = os.path.join(
        config["directories"]["results_dir"], config["name"], "final_loci_detection.tsv"
    )
    final_loci_df.to_csv(final_loci_output_path, sep="\t", index=True)
    logger.info(
        f"Saved final combined loci detection results to {final_loci_output_path}"
    )
    save_pickle(
        ...
        os.path.join(
            ...
            "loci_of_selection",
            "detection",
            "final_loci_detection_filtered.pickle",
        ),
    )
    save_pickle(
        ...
        os.path.join(
            ...
            "loci_of_selection",
            "detection",
            "final_loci_detection_filtered_widths.pickle",
        ),
    )
...
Examples:
  ...
  spice loci_assignment --config <path/to/config>
    ...
    subparsers = parser.add_subparsers(
        dest="mode", required=True, help="SPICE mode to run"
    )
    ...
    parser_event.add_argument(
        "--clean", action="store_true", help="Clean intermediate files and exit"
    )
    parser_event.add_argument(
        "--ids",
        type=str,
    ...
    parser_loci.add_argument(
        "--overwrite", action="store_true", help="Run new and overwrite existing data"
    )
    parser_loci.add_argument(
        "--snakemake",
        action="store_true",
