...
from spice.length_scales import DEFAULT_LENGTH_SCALE_BOUNDARIES, LS_I_DICT_REV
...
def run_loci_detection_per_chrom(
    ...
    if isinstance(which, str):
        if which == "default":
            ...
            assert which_start in which_options, (
                f"Invalid option {which_start} in {which}"
            )
            ...
            raise ValueError(
                f"Unknown which mode: {which}. Use 'default', 'fast', or a step name with '+'"
            )
    else:
    ...
    logger.info(
        f"Running loci detection for chrom={cur_chrom}, name={name} and a maximum of {N_loci} loci."
    )
    ...
    if "flipping" in which_steps:
        ...
        if RESULTS["detection"] is None:
            RESULTS["detection"], _, _ = open_pickle(
                os.path.join(output_dir, filenames["detection"])
            )
    ...
    if "ranking" in which_steps:
        ...
        if RESULTS["flipping"] is None:
            RESULTS["flipping"] = open_pickle(
                os.path.join(output_dir, filenames["flipping"])
            )

        if use_original_rank:
            logger.info(
                f"Using original rank from detection. Skipping rank_loci() function."
            )
    ...
    if "within_ci_filtering" in which_steps:
        ...
        if RESULTS["ranking"] is None:
            if use_original_rank:
                ...
                ranking_locus_iterations = open_pickle(
                    os.path.join(output_dir, filenames["ranking"])
                )
                RESULTS["ranking"] = ranking_locus_iterations[-1][0]
        ...
        RESULTS["within_ci_filtering"] = within_ci_fitness_filter(
            ...
            calc_new_force_new=overwrite,
            calc_new_filename=os.path.join(
                output_dir, filenames["within_ci_filtering"]
            ),
        )
    ...
    if "limiting" in which_steps:
        ...
        if RESULTS["within_ci_filtering"] is None:
            RESULTS["within_ci_filtering"] = open_pickle(
                os.path.join(output_dir, filenames["within_ci_filtering"])
            )
    ...
    if "optimizing_intermediate" in which_steps:
        ...
        log_debug(
            logger, f"Output: {output_dir}/{filenames['optimizing_intermediate']}"
        )

        if RESULTS["limiting"] is None:
            RESULTS["limiting"] = open_pickle(
                os.path.join(output_dir, filenames["limiting"])
            )
        ...
        RESULTS["optimizing_intermediate"], all_losses = final_optimization_step(
            ...
            calc_new_force_new=overwrite,
            calc_new_filename=os.path.join(
                output_dir, filenames["optimizing_intermediate"]
            ),
        )
    ...
    if "loci_widths_intermediate" in which_steps:
        ...
        log_debug(
            logger, f"Output: {output_dir}/{filenames['loci_widths_intermediate']}"
        )

        if RESULTS["optimizing_intermediate"] is None:
            RESULTS["optimizing_intermediate"] = open_pickle(
                os.path.join(output_dir, filenames["optimizing_intermediate"])
            )
    ...
    if "merging" in which_steps:
        ...
        if RESULTS["loci_widths_intermediate"] is None:
            RESULTS["loci_widths_intermediate"] = open_pickle(
                os.path.join(output_dir, filenames["loci_widths_intermediate"])
            )

        RESULTS["merging"], merged_conv, removed_loci, loci_to_remove = (
            merge_overlapping_loci(
                cur_chrom=cur_chrom,
    ...
    if "optimizing" in which_steps:
        ...
        if RESULTS[input_source] is None:
            RESULTS[input_source] = open_pickle(
                os.path.join(output_dir, filenames[input_source])
            )
    ...
    if "loci_widths_intermediate_2" in which_steps:
        ...
        log_debug(
            logger, f"Output: {output_dir}/{filenames['loci_widths_intermediate_2']}"
        )

        if RESULTS["optimizing"] is None:
            RESULTS["optimizing"] = open_pickle(
                os.path.join(output_dir, filenames["optimizing"])
            )
    ...
    if "filter_loci_intermediate_1" in which_steps:
        ...
        log_debug(
            logger, f"Output: {output_dir}/{filenames['filter_loci_intermediate_1']}"
        )

        if RESULTS["loci_widths_intermediate_2"] is None:
            RESULTS["loci_widths_intermediate_2"] = open_pickle(
                os.path.join(output_dir, filenames["loci_widths_intermediate_2"])
            )
        if RESULTS["optimizing"] is None:
            RESULTS["optimizing"] = open_pickle(
                os.path.join(output_dir, filenames["optimizing"])
        ...
        RESULTS["filter_loci_intermediate_1"] = filter_loci(
            ...
            calc_new_force_new=overwrite,
            calc_new_filename=os.path.join(
                output_dir, filenames["filter_loci_intermediate_1"]
            ),
        )
    ...
    if "final_within_ci_filtering" in which_steps:
        ...
        log_debug(
            logger, f"Output: {output_dir}/{filenames['final_within_ci_filtering']}"
        )
        ...
        RESULTS["final_within_ci_filtering"] = within_ci_fitness_filter(
            ...
            calc_new_force_new=overwrite,
            calc_new_filename=os.path.join(
                output_dir, filenames["final_within_ci_filtering"]
            ),
        )
    ...
    if "final_filter_loci" in which_steps:
        ...
        if RESULTS["final_within_ci_filtering"] is None:
            RESULTS["final_within_ci_filtering"] = open_pickle(
                os.path.join(output_dir, filenames["final_within_ci_filtering"])
            )
    ...
    if "final_limiting" in which_steps:
        ...
        if RESULTS["final_filter_loci"] is None:
            RESULTS["final_filter_loci"] = open_pickle(
                os.path.join(output_dir, filenames["final_filter_loci"])
            )
        ...
        RESULTS["final_selection_points"] = copy_list_of_selection_points(
            RESULTS["final_limiting"]
        )
        save_pickle(
            RESULTS["final_selection_points"],
            os.path.join(output_dir, filenames["final_selection_points"]),
        )
    ...
    if "final_loci_widths" in which_steps:
        ...
        if RESULTS["final_selection_points"] is None:
            RESULTS["final_selection_points"] = open_pickle(
                os.path.join(output_dir, filenames["final_selection_points"])
            )
...
def combine_loci(
    ...
    for i, cur_chrom in enumerate(CHROMS[:-1], 1):
        ...
        data_per_length_scale_file = os.path.join(
            loci_results_dir, "data_per_length_scale", f"{cur_chrom}.pickle"
        )
        ...
        final_selection_points_file = os.path.join(
            loci_results_dir, mode, cur_chrom, "final_selection_points.pickle"
        )
        if not os.path.exists(final_selection_points_file):
            logger.error(
                f"Missing final selection points for {cur_chrom} at {final_selection_points_file}"
            )
        ...
        # Load peak widths
        peak_widths_file = os.path.join(
            loci_results_dir, mode, cur_chrom, "final_loci_widths.pickle"
        )
    ...
    if calculate_p_value:
        ...
        filtered_selection_points, filtered_loci_widths, final_p_values = (
            full_filter_by_p_values(
        ...
        logger.info(
            f"Filtered to {sum(len(x) for x in filtered_loci_widths.values())} significant loci (p < {p_value_threshold})"
        )
    else:
    ...
    log_debug(
        logger,
        f"Final loci dataframe: {len(loci_df)} loci across {loci_df['chrom'].nunique()} chromosomes",
    )
...
def process_final_events_for_loci_routines(
    ...
    log_debug(
        logger,
        f"Loaded {len(final_events_df)} events events across {final_events_df['sample'].nunique()} samples and {final_events_df['id'].nunique()} IDs",
    )

    if remove_chrY:
        final_events_df = (
            final_events_df.query('chrom != "chrY"').reset_index(drop=True).copy()
        )

    # Remove IDs where the number of events does not match
    valid_ids = (
        final_events_df.groupby("id")
        .size()
        .loc[
            (
                final_events_df.groupby("id").size()
                == final_events_df.groupby("id")["events_per_chrom"].first()
            )
        ]
        .index.values
    )
    if len(valid_ids) < final_events_df["id"].nunique():
        logger.warning(
            f"Found {final_events_df['id'].nunique() - len(valid_ids)} IDs ({100 * (final_events_df['id'].nunique() - len(valid_ids)) / final_events_df['id'].nunique():.4f}%) with inconsistent number of events"
        )
        final_events_df = final_events_df.query("id in @valid_ids").copy()
        log_debug(
            logger,
            'Removed invalid IDs, where the number of events did not match "events_per_chrom"',
        )
        log_debug(
            logger,
            f"Events now have: {final_events_df['sample'].nunique()} samples, {final_events_df['id'].nunique()} IDs and {len(final_events_df)} events",
        )

    final_events_df = (
        final_events_df.loc[~final_events_df["telomere_bound"]]
    ...
    final_events_df = final_events_df.join(
        data_loaders.load_centromeres(extended=True), on="chrom"
    )
    final_events_df[
        [
            "centromere_bound_l",
        ...
        calc_telomere_bound_whole_arm_whole_chrom(
            final_events_df, return_left_and_right=True
        ),
        axis=1,
    )
    ...
    final_events_df.loc[final_events_df["telomere_bound_r"], "end"] = (
        final_events_df.loc[final_events_df["telomere_bound_r"], "chrom_length"]
    )

    final_events_df["whole_arm"] = final_events_df.eval(
        "(telomere_bound_l and centromere_bound_r) or (telomere_bound_r and centromere_bound_l)"
    )
    final_events_df.loc[final_events_df.query("whole_chrom").index, "whole_arm"] = False

    final_events_df["centromere_bound"] = np.logical_or(
        final_events_df["centromere_bound_l"].values,
        final_events_df["centromere_bound_r"].values,
    )
    final_events_df["whole_centromere"] = np.logical_and(
        final_events_df["centromere_bound_l"].values,
        final_events_df["centromere_bound_r"].values,
    )
    ...
    log_debug(
        logger,
        "Remove whole centromere events and events within 1Mbp of the centromere",
    )
    centromeres = data_loaders.load_centromeres(extended=True)
    final_events_df = final_events_df.drop(
        columns=["centro_start", "centro_end"], errors="ignore"
    ...
    final_events_df = final_events_df.drop(
        columns=["centro_start_pad", "centro_end_pad"], errors="ignore"
    ).join(centromeres_pad, on="chrom")
    final_events_df["inside_centromere"] = final_events_df.eval(
        "start>=centro_start_pad-2 and end<=centro_end_pad+2"
    )

    final_events_df = (
        final_events_df.query("not whole_centromere and not inside_centromere")
        .drop(columns=["whole_centromere", "inside_centromere"])
        .copy()
    )
    assert len(final_events_df) > 0, (
        "No events left after removing whole centromeres and events within 1Mbp of the centromere. Please check the centromere definitions and event coordinates."
    )
    log_debug(
        logger,
        f"Events now have: {final_events_df['sample'].nunique()} samples, {final_events_df['id'].nunique()} IDs and {len(final_events_df)} events",
    ...
    final_events_df.loc[
        final_events_df.query("chrom in @short_chroms").index, "short_arm"
    ] = True
    final_events_df.loc[
        final_events_df.query("chrom in @short_chroms and whole_arm").index,
        "whole_chrom",
    ] = True
    final_events_df.loc[
        final_events_df.query("chrom in @short_chroms and whole_arm").index, "whole_arm"
    ] = False
    final_events_df.loc[
        final_events_df.query(
            "chrom in @short_chroms and telomere_bound_r and start <= centro_end_pad+2"
        ).index,
        "whole_chrom",
    ] = True

    # Events that are within 0.95-1.05 of the arm size are considered whole arm
    telomere_bound_events = final_events_df.query(
        "telomere_bound and not whole_chrom and not whole_arm"
    ).copy()
    telomere_bound_events["left_bound"] = telomere_bound_events.eval("start <= 100000")
    telomere_bound_events["right_bound"] = telomere_bound_events.eval(
        "end >= chrom_length - 100000"
    )
    telomere_bound_events.loc[
        telomere_bound_events["chrom"].isin(short_chroms), "left_bound"
    ] = telomere_bound_events.loc[
        telomere_bound_events["chrom"].isin(short_chroms)
    ].eval("start <= centro_end")
    telomere_bound_events["arm_size"] = telomere_bound_events["centro_start"]
    telomere_bound_events.loc[telomere_bound_events["right_bound"], "arm_size"] = (
        telomere_bound_events.loc[telomere_bound_events["right_bound"], "chrom_length"]
        - telomere_bound_events.loc[telomere_bound_events["right_bound"], "centro_end"]
    )
    telomere_bound_events.loc[
        telomere_bound_events["chrom"].isin(short_chroms), "arm_size"
    ] = (
        telomere_bound_events.loc[
            telomere_bound_events["chrom"].isin(short_chroms), "chrom_length"
        ]
        - telomere_bound_events.loc[
            telomere_bound_events["chrom"].isin(short_chroms), "centro_end"
        ]
    )
    telomere_bound_events["within_arm"] = (
        telomere_bound_events["width"] < telomere_bound_events["arm_size"]
    )
    telomere_bound_events["width_norm"] = (
        telomere_bound_events["width"] / telomere_bound_events["arm_size"]
    )
    whole_chrom_indices = telomere_bound_events.query(
        "(left_bound and right_bound)"
    ).index.values
    whole_arm_indices = telomere_bound_events.query(
        "not (left_bound and right_bound) and width_norm > 0.95 and width_norm < 1.05"
    ).index.values
    ...
    final_events_df["pos"] = final_events_df.apply(
        lambda x: (
            ...
            if x["whole_arm"]
            else "centromere_bound"
            if x["centromere_bound"]
            else "telomere_bound"
            if x["telomere_bound"]
            else "internal"
        ),
    ...
    if use_observed_centromeres:
        ...
        for cur_chrom in final_events_df["chrom"].unique():
            for cur_length_scale in ["small", "mid1", "mid2", "large"]:
                ...
                cur_events = final_events_df.query(
                    'chrom == @cur_chrom and pos == "internal" and width > @cur_length_scale_border[0] and width <= @cur_length_scale_border[1]'
                )
                is_centromere_bound_new = (
                    (
                        cur_events["start"]
                        >= CENTROMERES_OBSERVED[cur_length_scale].loc[
                            cur_chrom, "centro_start"
                        ]
                    )
                    & (
                        cur_events["start"]
                        <= CENTROMERES_OBSERVED[cur_length_scale].loc[
                            cur_chrom, "centro_end"
                        ]
                    )
                ) | (
                    (
                ...
                cur_ind = cur_events.loc[
                    (is_centromere_bound_new & ~cur_events["telomere_bound"])
                ].index
        ...
        log_debug(
            logger,
            f"Assigned {new_n_centromere - old_n_centromere} new events as centromere bound using observed centromeres",
        )
    ...
    final_events_df = final_events_df.query(
        'pos == "internal" and width >= @min_width and width <= @max_width'
    ).copy()
    log_debug(
        logger,
        f"Only kept internal events: {len(final_events_df)} remaining (dropped {old_n - len(final_events_df)})",
    )
    ...
    if drop_duplicates:
        ...
        final_events_df = final_events_df.drop_duplicates(
            ["id", "chrom", "type", "start", "end"], keep="first"
        ).copy()
        log_debug(
            logger,
            f"Dropped {old_len - len(final_events_df)} duplicates -> {len(final_events_df)} events",
        )
    ...
    if config["input_files"].get("plateaus", None) is not None:
        ...
        plateaus_df = pd.read_csv(
            config["input_files"]["plateaus"], sep="\t", index_col=None
        )
        log_debug(logger, f"Loaded plateaus data: {len(plateaus_df)} entries")
        if plateaus_df is not None:
            final_events_df = categorize_events_by_plateau_overlap(
                plateaus_df, final_events_df
            )
            log_debug(
                logger,
                f"Categorized {len(final_events_df)} events by plateau overlap ({len(plateaus_df)} plateaus): {dict(final_events_df['plateau'].value_counts())}",
            )
            if remove_plateaus:
                plateau_events = final_events_df.query(
                    'plateau != "neither_left_nor_right"'
                )
                log_debug(
                    logger,
                    f"Filtering out {len(plateau_events)} events overlapping plateaus",
                )
                final_events_df = (
...
def run_loci_assignment_per_chrom(
    ...
    save_pickle(
        ...
        os.path.join(output_dir, "final_selection_points.pickle"),
    )
...
def loci_assignment(
    ...
    if reference_loci_file and os.path.exists(reference_loci_file):
        ...
        try:
            ...
            content = (
                files("spice").joinpath("reference_loci", resource_name).read_text()
            )
    ...
    if missing_cols:
        raise ValueError(
            f"reference_loci must have columns {required_cols}, missing: {missing_cols}"
        )
    ...
    if invalid_types:
        raise ValueError(
            f"type column must contain only 'OG' or 'TSG', found: {invalid_types}"
        )

    chromosomes = processed_events["chrom"].unique()
    assert set(chromosomes).issubset(
        set(["chr" + str(x) for x in range(1, 23)] + ["chrX", "chrY"])
    ), (
        f"Unexpected chromosomes in final events: {set(chromosomes) - set(['chr' + str(x) for x in range(1, 23)] + ['chrX', 'chrY'])}"
    )
    logger.info(
        f"Found {len(chromosomes)} unique chromosomes in final events: {chromosomes}"
    )
...
def build_loci_sample_matrix(
    ...
    Binary matrix:
        A cell is 1 if the sample has at least one event of the matching type
        whose genomic range contains the locus center position, 0 otherwise.

    Weighted matrix (fitness-weighted attribution):
        Uses the same fitness-weighted normalisation as calc_event_rate_per_loci.
        Events are split into 8 (length_scale, type) layers. For each layer and
        chromosome, the fractional attribution of every event to each locus is:

            attr(event e, locus j) =
                fitness_j * overlap(e, j)
                ─────────────────────────────────────────────────
                sum_{j'} [ fitness_{j'} * overlap(e, j') ] + 1

        where overlap(e, j) = 1 iff event.start < locus.pos < event.end.
        The +1 in the denominator is a baseline term (identical to the
        convention in calc_event_rate_per_loci).

        The weighted cell W[j, s] is the sum of these attributions over all
        events belonging to sample s and over all 8 layers.

        Verification property:
            sum_s W[j, s]  ≈  final_loci_df["added_events"][j]

    Overlap criterion (consistent with calc_event_rate_per_loci):
        event.start < locus.pos < event.end   (point containment)

    Type mapping per layer:
        layer gain  ->  reads fitness_{ls}_gain  columns of final_loci_df
        layer loss  ->  reads fitness_{ls}_loss  columns of final_loci_df

    Parameters
    ----------
    final_loci_df : pd.DataFrame
        Loci dataframe (indexed by locus index) with columns:
        chrom, pos, type (OG or TSG), and fitness_{ls}_{type} for each of the
        8 (length_scale, type) combinations produced by create_loci_df().
    processed_events : pd.DataFrame
        Processed events (output of process_final_events_for_loci_routines),
        with columns: sample, chrom, start, end, type (gain or loss), width.
    ...
    Tuple[pd.DataFrame, pd.DataFrame]
        ...
        weighted_matrix: float32 - fitness-weighted attribution per locus/sample.
    """
    # 8 (length_scale, type) layers, matching LS_I_DICT_REV ordering
    ls_keys = [(LS_I_DICT_REV[i][0], LS_I_DICT_REV[i][1]) for i in range(8)]
    ...
    # Reset loci_df to a positional index for array indexing inside the loop
    loci_reset = final_loci_df.reset_index(drop=True)

    binary_data = np.zeros((n_loci, n_samples), dtype=np.int8)
    weighted_data = np.zeros((n_loci, n_samples), dtype=np.float32)

    # ------------------------------------------------------------------ #
    # Binary matrix: same point-containment logic as before, but          #
    # combined across all layers (any event of matching type suffices).   #
    # ------------------------------------------------------------------ #
    type_map = {"OG": "gain", "TSG": "loss"}
    for row_i, locus in loci_reset.iterrows():
        ...
        if overlapping.empty:
            continue
        for s in overlapping["sample"].unique():
            if s in sample_idx:
                binary_data[row_i, sample_idx[s]] = 1

    # ------------------------------------------------------------------ #
    # Weighted matrix: fitness-weighted attribution per layer              #
    # ------------------------------------------------------------------ #
    for cur_ls, cur_type in ls_keys:
        fitness_col = f"fitness_{cur_ls}_{cur_type}"
        if fitness_col not in loci_reset.columns:
            continue

        width_lo, width_hi = DEFAULT_LENGTH_SCALE_BOUNDARIES[cur_ls]
        # Filter events to this layer (width bucket + event type)
        layer_mask = (
            (processed_events["type"] == cur_type)
            & (processed_events["width"] > width_lo)
            & (processed_events["width"] <= width_hi)
        )
        layer_events = processed_events.loc[layer_mask]
        if layer_events.empty:
            continue

        # Precompute sample index array for layer events (avoids repeated dict look-ups)
        evt_sample_j = np.array(
            [sample_idx.get(s, -1) for s in layer_events["sample"].values]
        )

        # Process per chromosome (attribution normalisation is within-chromosome)
        for cur_chrom in layer_events["chrom"].unique():
            chrom_loci_mask = loci_reset["chrom"].values == cur_chrom
            chrom_loci_idx = np.where(chrom_loci_mask)[
                0
            ]  # positional indices into loci arrays
            if len(chrom_loci_idx) == 0:
                continue

            loci_pos = loci_reset["pos"].values[chrom_loci_idx]  # shape (L,)
            loci_fit = np.maximum(
                0.0, loci_reset[fitness_col].values[chrom_loci_idx]
            )  # shape (L,)

            chrom_evt_mask = layer_events["chrom"].values == cur_chrom
            chrom_evt_idx = np.where(chrom_evt_mask)[0]  # positional into layer_events
            if len(chrom_evt_idx) == 0:
                continue

            evt_start = layer_events["start"].values[chrom_evt_idx]  # shape (E,)
            evt_end = layer_events["end"].values[chrom_evt_idx]  # shape (E,)
            evt_sj = evt_sample_j[chrom_evt_idx]  # shape (E,), sample column idx

            # Overlap matrix: overlap[e, l] = 1 iff event e covers locus l
            # shape (E, L)
            overlaps = (evt_start[:, None] < loci_pos[None, :]) & (
                evt_end[:, None] > loci_pos[None, :]
            )

            # Fitness-weighted attribution, identical to calc_event_rate_per_loci:
            #   attr[e, l] = overlap[e,l] * fitness[l]
            #                ─────────────────────────────
            #                sum_l(overlap[e,l]*fitness[l]) + 1
            weighted_overlaps = (
                overlaps.astype(np.float64) * loci_fit[None, :]
            )  # (E, L)
            row_sums = weighted_overlaps.sum(axis=1) + 1.0  # (E,)
            attr = weighted_overlaps / row_sums[:, None]  # (E, L)

            # Scatter-add attribution into weighted_data per sample.
            # Iterate over unique sample column indices present in this chunk.
            valid_evt_mask = evt_sj >= 0
            if not valid_evt_mask.any():
                continue

            for sj in np.unique(evt_sj[valid_evt_mask]):
                evt_sel = valid_evt_mask & (evt_sj == sj)
                # Sum attributions over selected events for this sample → shape (L,)
                attr_sum = attr[evt_sel].sum(axis=0)
                weighted_data[chrom_loci_idx, sj] += attr_sum.astype(np.float32)
...
def build_final_loci_df(
    ...
    loci_df = calculate_events_per_loci_df(
        loci_df,
        all_selection_points=all_selection_points,
        final_events_df=final_events_df,
    )
...
def full_filter_by_p_values(
    ...
    if loci_df is None:
        assert final_events_df is not None, (
            "final_events_df must be provided if loci_df is None"
        )
        loci_df = create_loci_df(
            all_selection_points,
            all_loci_widths,
    ...
    logger.info(
        f"Out of {len(loci_df)} loci, {len(loci_df.query('p_value < @p_value_threshold'))} ({len(loci_df.query('p_value < @p_value_threshold')) / len(loci_df):.2%}%) are significant at p < {p_value_threshold} "
    )
    ...
    for cur_chrom in loci_df["chrom"].unique():
        cur_sp = [
            list(x)
            for x in copy_list_of_selection_points(all_selection_points[cur_chrom])
        ]
        cur_loci = loci_df.query("chrom == @cur_chrom")
        is_significant = (
            cur_loci.sort_values("rank_on_chrom")
            .eval("p_value < @p_value_threshold")
            .values
        )
        ...
        final_p_values.append(
            cur_loci.sort_values("rank_on_chrom").loc[is_significant]["p_value"].values
        )
    final_p_values = np.concatenate(final_p_values)
    assert np.all(final_p_values < p_value_threshold), (
        f"{np.sum(final_p_values >= p_value_threshold)} loci with p >= {p_value_threshold} after filtering"
    )
    ...
    for cur_chrom in loci_df["chrom"].unique():
        ...
        allowed_fitness_change = np.stack(
            [[x[0].fitness != 0 for x in y] for y in cur_sp]
        )
        up_down_order = np.array(
            [
                any([cur_sp[j][cluster_j][0].fitness > 0 for j in range(0, 8, 2)])
                for cluster_j in range(len(cur_sp[0]))
            ]
        )
        cur_conv = convolution_simulation_per_ls(
            cur_chrom, all_data_per_length_scale[cur_chrom], cur_sp
        )
