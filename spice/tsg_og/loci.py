...
def create_loci_df(
    ...
    if all_loci_widths is None:
        all_loci_widths = {
            cur_chrom: len(all_selection_points[cur_chrom][0]) * [[0]]
    ...
    loci_df = pd.DataFrame(
        {
            "chrom": np.concatenate(
                [
                    [cur_chrom] * len(chrom_selection_points[0])
                    for cur_chrom, chrom_selection_points in all_selection_points.items()
                ]
            ),
            "pos": np.concatenate(
                [
                    [x[0].pos for x in chrom_selection_points[0]]
                    for chrom_selection_points in all_selection_points.values()
                ]
            ),
            "rank_on_chrom": np.concatenate(
                [
                    [i for i in range(len(cur_selection_points[0]))]
                    for cur_selection_points in all_selection_points.values()
                ]
            ),
            "width": np.concatenate(
                [
                    [np.std(x) for x in chrom_loci_widths]
    ...
    if len(loci_df) == 0:
        raise ValueError(
            "No loci found. Please check your data and the input parameters."
        )
    loci_df["chrom"] = data_loaders.format_chromosomes(loci_df["chrom"])
    loci_df["pos_total"] = (
        loci_df.join(
            CHROM_LENS.cumsum().shift(1, fill_value=0).to_frame("chrom_offset"),
            on="chrom",
        )
        .eval("pos + chrom_offset")
        .values
    )
    ...
    loci_df[
        [f"fitness_{LS_I_DICT_REV[i][0]}_{LS_I_DICT_REV[i][1]}" for i in range(8)]
    ] = np.concatenate(
        [
            np.stack(
                [
                    [ls[0].fitness for ls in locus]
                    for locus in list(zip(*chrom_selection_points))
                ]
            )
            for chrom_selection_points in all_selection_points.values()
        ],
    ...
    loci_df["type"] = np.where(
        (
            loci_df[
                [
                    f"fitness_{LS_I_DICT_REV[i][0]}_{LS_I_DICT_REV[i][1]}"
    ...
    # 'up_down' mirrors 'type' and is used by merge_overlapping_loci to group
    # loci of the same direction (OG = gain/up, TSG = loss/down) when detecting
    # overlap candidates.
    loci_df["up_down"] = loci_df["type"]
...
def overlap_with_cosmic_davoli(loci_df, cosmic_loci, davoli_loci):
    ...
    loci_df["cosmic_genes"] = [
        [cosmic_loci["Gene Symbol"].values[y] for y in np.where(x)[0]]
        for x in cosmic_overlaps.T
    ]
    loci_df["davoli_genes"] = [
        [davoli_loci["name"].values[y] for y in np.where(x)[0]]
        for x in davoli_overlaps.T
    ]
    loci_df["cosmic_davoli_genes"] = loci_df.apply(
        lambda x: (
            np.intersect1d(x["cosmic_genes"], x["davoli_genes"]).tolist()
            if x["cosmic"] > 0 and x["davoli"] > 0
            else []
        ),
        axis=1,
    )
...
def overlap_with_gistic_biscut(loci_df, gistic_loci, biscut_loci):
    ...
    loci_df["gistic_genes"] = loci_df["gistic_loci"].map(
        lambda x: sum(
            [
                []
                if isinstance(gistic_loci["genes"].values[y], float)
                else gistic_loci["genes"].values[y]
                for y in x
            ],
            [],
        )
    ...
    loci_df["biscut_genes"] = loci_df["biscut_loci"].map(
        lambda x: sum(
            [
                []
                if isinstance(biscut_loci["genes"].values[y], float)
                else biscut_loci["genes"].values[y]
                for y in x
            ],
            [],
        )
    )
    loci_df["gistic_biscut_genes"] = loci_df.apply(
        lambda x: (
            np.intersect1d(x["gistic_genes"], x["biscut_genes"]).tolist()
            if x["gistic"] > 0 and x["biscut"] > 0
            else []
        ),
        axis=1,
    )
...
def calculate_events_per_loci_df(
    loci_df,
    all_selection_points=None,
    final_events_df=None,
    rates_and_events_per_loci=None,
):

    loci_df["added_events"] = 0.0
    for cur_chrom in CHROMS[:-1]:
        if rates_and_events_per_loci is not None:
            ...
            if (
                len(final_events_df.query('chrom == @cur_chrom and pos == "internal"'))
                == 0
            ):
                continue
        ...
        cur_added_events = [
            x[:-1] for x in total_events_per_loci.values() if not isinstance(x, int)
        ]
        if len(cur_added_events) == 0:
            continue
        cur_added_events = np.sum(np.stack(cur_added_events), axis=0)
        cur_index = (
            loci_df.query("chrom == @cur_chrom").sort_values("rank_on_chrom").index
        )
...
def calc_prominence(
    ...
    if cur_loci_df is None:
        assert selection_points is not None and loci_widths is not None, (
            "If cur_loci_df is None, final_selection_points and final_loci_widths must be provided"
        )
        cur_loci_df = create_loci_df(
            all_selection_points={cur_chrom: selection_points},
    ...
    for cur_length_scale in ["small", "mid1", "mid2", "large"]:
        for cur_type in ["gain", "loss"]:
            ...
            cur_kernel_size = data_per_length_scale[(cur_length_scale, cur_type)][
                "loci_width"
            ]

            if calc_on == "data":
                data_for_prominence = data_per_length_scale[
                    (cur_length_scale, cur_type)
                ]["signals"]
                data_for_prominence = gaussian_filter1d(
                    data_for_prominence, size_neighborhood
                )
            elif calc_on == "conv":
                data = data_per_length_scale[(cur_length_scale, cur_type)]
                if selection_points is None:
                    cur_selection_points = selection_points_from_loci_df(
                        cur_loci_df, cur_chrom, ls_i
                    )
                else:
                ...
                data_for_prominence = convolution_simulation(
                    cur_chrom=cur_chrom,
                    selection_points=combine_selection_points(cur_selection_points),
    ...
    cur = cur_loci_df[
        [col for col in cur_loci_df.columns if col.startswith("prominence_OG")]
    ].values
    cur[
        cur_loci_df[
            [
                x
                for x in cur_loci_df.columns
                if x.startswith("fit_") and x.endswith("gain")
            ]
        ].values
        <= 0
    ] = 0
    ...
    cur = cur_loci_df[
        [col for col in cur_loci_df.columns if col.startswith("prominence_TSG")]
    ].values
    cur[
        cur_loci_df[
            [
                x
                for x in cur_loci_df.columns
                if x.startswith("fit_") and x.endswith("loss")
            ]
        ].values
        <= 0
    ] = 0
    ...
    # set max_prominence
    cur_loci_df["max_prominence"] = cur_loci_df[
        [col for col in cur_loci_df.columns if col.startswith("prominence_OG")]
    ].max(axis=1)
    cur_loci_df.loc[cur_loci_df["type"] == "TSG", "max_prominence"] = cur_loci_df.loc[
        cur_loci_df["type"] == "TSG"
    ][[col for col in cur_loci_df.columns if col.startswith("prominence_TSG")]].max(
        axis=1
    )
    cur_loci_df["sum_prominence"] = cur_loci_df[
        [col for col in cur_loci_df.columns if col.startswith("prominence")]
    ].sum(axis=1)
...
def process_locus_prominence(
    ...
    cur_loci_df[f"prominence_{cur_loci_type}_{cur_length_scale}"] = 0.0
    cur_loci_df_ud = cur_loci_df.query("type == @cur_loci_type")
    for i, ind in enumerate(cur_loci_df_ud.index):
        cur_pos_raw = int(
            cur_loci_df_ud.iloc[i]["pos"] / segment_size_dict[cur_length_scale]
        )
        # Choose the std start / end values
        cur_start = int(
            cur_loci_df_ud.iloc[i]["start"] / segment_size_dict[cur_length_scale]
        )
        cur_end = int(
            cur_loci_df_ud.iloc[i]["end"] / segment_size_dict[cur_length_scale]
        )
        ...
        assert cur_start < cur_end, (
            f"Start ({cur_start}) must be less than end ({cur_end}) for locus {ind}"
        )
        cur_pos = cur_start + np.argmax(
            data_for_prominence[
                max(0, cur_start) : min(cur_end, len(data_for_prominence) - 1)
            ]
        )
        ...
        cur_loci_df.loc[ind, f"prominence_{cur_loci_type}_{cur_length_scale}"] = (
            cur_prominence
        )
        ...
        if actual_loci_width > cur_kernel_size * 2:
            cur_loci_df.loc[ind, f"prominence_{cur_loci_type}_{cur_length_scale}"] = 0.0

    return cur_loci_df


def calc_locus_prominence_on_combined_conv(
    loci_df,
    all_simulated_conv_combined,
    ...
    for cur_chrom in CHROMS[:-1]:
        ...
        for cur_type in ["gain", "loss"]:
            ...
            cur_kernel_size = all_data_per_length_scale[cur_chrom][("large", cur_type)][
                "loci_width"
            ]
            ...
            cur_loci_df[f"combined_prominence_{cur_type}"] = cur_loci_df_[
                f"prominence_{cur_type}_{cur_length_scale}"
            ]
        cur_loci_df["combined_prominence"] = cur_loci_df[
            [f"combined_prominence_{t}" for t in ["gain", "loss"]]
        ].max(axis=1)
    ...
    assert np.all(
        (
            all_loci.sort_index()[["chrom", "pos"]].values
            == loci_df.sort_index()[["chrom", "pos"]].values
        )
    )
...
def prominence_overlap_check(
    ...
    centro_telo_pos = np.concatenate(
        [CENTROMERES.values.T.flatten(), len(CHROMS) * [0], CHROM_LENS.values]
    )
    centro_telo_chrom = np.concatenate(
        [2 * list(CENTROMERES.index), CHROMS, CHROM_LENS.index]
    )
    assert len(centro_telo_pos) == len(centro_telo_chrom)
    locus_start = (
        locus_pos
        - data_per_length_scale[("mid1", "gain")]["loci_width"]
        * segment_size_dict["mid1"]
        / 2
    )
    locus_end = (
        locus_pos
    ...
    loci_within_centro_telo = reduce(
        np.logical_and,
        [
    ...
    loci_to_remove_with_replacing_locus = loci_to_remove[
        np.logical_or(
            loci_within_other_loci.any(axis=1), loci_within_centro_telo.any(axis=1)
        )
    ]
    loci_df["prominence_within_other_locus"] = False
    loci_df.loc[
        loci_to_remove_with_replacing_locus, "prominence_within_other_locus"
    ] = True
...
def selection_points_from_loci_df(loci_df, cur_chrom, ls_i):
    return [
        SelectionPoints(loci=[x])
        for x in loci_df.query("chrom == @cur_chrom")[["pos", f"fit_{ls_i}"]].values
    ]
...
def assign_p_values(
    ...
    from spice.tsg_og.p_values import (
        p_value_using_resim,
        get_actual_p_values_from_results,
    )

    log_debug(
        logger,
        f"Assigning p-values to loci for {len(data_per_length_scale)} chromosomes with N_random={N_random}, n_iterations_optim={n_iterations_optim}, overwrite={overwrite}",
    )

    loci_df["p_value_raw"] = 1.0
    for cur_chrom in data_per_length_scale.keys():
        for cur_type in ["OG", "TSG"]:
            ...
            if overwrite or not os.path.exists(p_values_cache_file):
                logger.info(
                    f"Calculating p-value distribution for {cur_chrom} ({cur_type})"
                )
            ...
            p_values = get_actual_p_values_from_results(
                cur_loci, p_value_results, N_random
            )
