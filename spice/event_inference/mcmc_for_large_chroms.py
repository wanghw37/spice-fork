...
from spice.event_inference.events_from_graph import (
    get_starts_and_ends,
    create_random_start_end_pairs,
    get_events_diff_from_coords,
    get_events_diff_from_coords_wgd,
    loh_filters_for_graph_result_diffs,
    loh_filters_for_graph_result_diffs_wgd,
    connect_start_ends_using_svs,
    get_wgd_single_solution,
    _deepcopy_fast,
)
...
class ZeroWidthEventError(ValueError):
    """Raised when an MCMC proposal contains a zero-width event.

    Root cause: 1bp gaps in raw CNA data (e.g. end=N, next_start=N+1) are
    converted to zero-gap segment boundaries (end[i] == start[i+1]) during
    centromere processing via half-open interval convention.  If a proposal
    function generates an event with start_idx == end_idx pointing to such a
    boundary, get_event_dist_data_from_mcmc_proposal computes width = 0.
    """
...
Mcmc_result = namedtuple(
    ...
    [
        ...
        "best_diffs",
        "best_sv_overlaps",
        "is_accepted_iteration",
        "scores_full",
        "sv_overlaps_full",
    ],
)
Mcmc_result_full = namedtuple(
    "Mcmc_result",
    [
        "cur_id",
        "best_score",
...
def mcmc_event_selection(
    ...
    if __version__ != "9.8.3296":
        raise ValueError(
            f"ortools must have version 9.8.3296, current version is {__version__}"
        )
    ...
    assert len(cn_profile) == len(cur_chrom_segments), (
        f"cn_profile ({len(cn_profile)}) and cur_chrom_segments ({len(cur_chrom_segments)}) do not have the same lengths"
    )
    has_loh = (cn_profile == 0).any()
    if not has_loh:
        perform_loh_checks = False
    starts, ends = get_starts_and_ends(
        cn_profile, prior_profile=None, loh_adjust=True, total_cn=total_cn
    )
    n_events = len(starts)
    if len(starts) == 0:
        return None
    ...
    log_debug(
        logger,
        f"MCMC for {cur_id}. CN profile: {cn_profile}. {n_events} events. {'WGD' if has_wgd else 'no WGD'}.",
    )

    (
        all_scores,
        all_sv_overlaps,
        all_loh_filter_passed,
        all_best_events,
        all_best_diffs,
        is_accepted_iteration,
    ) = [], [], [], [], [], []
    ...
    if sv_data is not None:
        ...
        cur_sv_data = sv_data.query(
            'chrom_id == @chrom_id and (svclass == "DUP" or svclass == "DEL")'
        ).copy()
        if has_wgd:
            all_starts, all_ends = (
                np.arange(len(cn_profile)),
                np.arange(1, len(cn_profile) + 1),
            )
            ...
            sv_selected_events_set = (
                set([tuple(x) for x in sv_selected_events])
            ...
            if sv_selected_events is not None and len(sv_selected_events) > 0:
                n_sv_overlaps = (
                    len(sv_selected_events) if sv_selected_events.size > 0 else 0
                )
                ...
                sv_selected_data, sv_selected_score = calc_event_distances_mcmc_wrapper(
                    ...
                    knn_graph_k,
                    cur_chrom,
                    cur_segment_breakpoints_starts,
                    cur_segment_breakpoints_ends,
                    cn_profile,
                    centro_start,
                    centro_end,
                    cur_chrom_length,
                    chrom_data.has_wgd,
                    log10_distances=log10_distances,
                    wgd_split=wgd_split,
                )
                if sv_selected_score is None:
                    logger.warning(
                        "SV-selected events contain zero-width event; disabling SV pre-selection"
                    )
                    sv_selected_score = 0
                    sv_selected_events = None
                    n_sv_overlaps = 0
                    best_sv_overlaps = 0
                    sv_selected_events_set = set()
                else:
    ...
    log_debug(
        logger,
        f"{n_sv_overlaps} SV overlaps for a total of {n_events} events. {len(starts)} events remaining",
    )
    ...
    if n_sv_overlaps > 0 and len(starts) <= 1:
        ...
        if len(starts) == 1:
            sv_selected_events = np.concatenate(
                [sv_selected_events, np.array([[starts[0], ends[0]]])], axis=0
            )
    ...
    if has_wgd:
        ...
        starts, ends = (
            np.array(starts),
            np.array(ends),
        )  # for whatever reason, it doesn't work without this
        ...
        if n_sv_overlaps > 0 and 0 in cn_profile:
            ...
            if len(diffs) == 0:
                log_debug(
                    logger,
                    "Initial solution using SVs was not compatible with LOHs -> rerunning without SVs",
                )
                sv_selected_score = 0
                sv_selected_events_set = []
                sv_selected_events = None
                ...
                event_proposal = np.array(
                    create_random_start_end_pairs(starts, ends, 1)[0]
                )
    ...
    for iteration in tqdm(range(n_iterations), disable=not show_progress):
        logger.debug(
            f"iteration {iteration} / {n_iterations} ({int(100 * iteration / n_iterations):d}%). Best score: {overall_best_score}"
        )
        ...
        if iteration != 0:  # needed in case the first guess is already optimal
            assert not has_wgd or len(best_events) == 2, (
                f"best events has wrong length: {len(best_events)}"
            )
            event_proposal = create_mcmc_proposal(
                ...
                iteration,
                cn_profile,
                swap_event_based_on_score,
                event_distances,
                total_cn=total_cn,
            )
            assert not has_wgd or len(event_proposal) == 2, (
                f"event proposal has wrong length: {len(event_proposal)}"
            )
        ...
        if loh_check_before_distance_calculation and 0 in cn_profile:
            ...
            if len(diffs) == 0:
                logger.debug(
                    f"{iteration}: proposal rejected because early loh filter failed"
                )
                all_scores.append(all_scores[-1] if len(all_scores) > 0 else np.inf)
                all_sv_overlaps.append(
                    all_sv_overlaps[-1] if len(all_sv_overlaps) > 0 else 0
                )
                continue
            event_proposal = _get_events_from_diff(diffs[0], has_wgd)
            if has_wgd:
                event_proposal = [
                    [tuple(x) for x in event_proposal[0]],
                    [tuple(x) for x in event_proposal[1]],
                ]
            else:
        ...
        if event_distances is None:
            # Proposal contains a zero-width event; skip this MCMC iteration silently.
            logger.debug(
                f"{iteration}: proposal rejected due to zero-width event (zero-gap segment boundary)"
            )
            all_scores.append(all_scores[-1] if len(all_scores) > 0 else np.inf)
            all_sv_overlaps.append(
                all_sv_overlaps[-1] if len(all_sv_overlaps) > 0 else 0
        ...
        if has_wgd and any(
            [
                (pre_event[1], pre_event[0]) in event_proposal[1]
                for pre_event in event_proposal[0]
            ]
        ):
            logger.debug(
                f"{iteration}: proposal rejected because it contains a doubled gain/loss pair"
            )
            all_scores.append(all_scores[-1] if len(all_scores) > 0 else np.inf)
            all_sv_overlaps.append(
                all_sv_overlaps[-1] if len(all_sv_overlaps) > 0 else 0
        ...
        if has_wgd and len(sv_selected_events_set) > 0:
            cur_n_sv_overlaps = len(
                sv_selected_events_set.intersection(set(event_proposal[0]))
            ...
            if cur_n_sv_overlaps > n_sv_overlaps:
                ...
                logger.debug(
                    f"{iteration}: proposal accepted because of SV overlap increase ({n_sv_overlaps})"
                )
                force_accept = True

        # accept / reject and process
        if simulated_annealing:
            assert max_T - min_T < 0 and max_T < min_T, (
                f"min_T and max_T must be negative and min_T < max_T. ({min_T} {max_T})"
            )
        ...
        acceptance = (
            ...
            or (
                simulated_annealing
                and (np.random.random() < np.exp((best_score - cur_score) / T))
            )
            or (
                not simulated_annealing
                and (
                    np.random.random()
                    < np.exp((best_score - cur_score) / acceptance_temp)
                )
            )
        )
        ...
        if acceptance:
            logger.debug(
                f"{iteration}: proposal accepted (old: {best_score}, new: {cur_score})"
            )

            if (
                sv_data is not None and sv_selected_events is not None and not has_wgd
            ):  # note this can only happen for nowgd
                event_proposal = np.concatenate([sv_selected_events, event_proposal])

            if (
                diffs is None
            ):  # only calculate diffs weren't already created in the loop
                diffs = create_diff_and_check_loh(
                    event_proposal,
                    cn_profile,
                    has_wgd,
            ...
            if len(diffs) != 1:
                logger.debug(
                    f"{iteration}: no LOH viable solution found, rejecting proposal"
                )
            ...
            if (
                force_accept  # force accept = WGD and more SV overlaps
                or (
                    (cur_score < overall_best_score)
                    and (cur_n_sv_overlaps >= best_sv_overlaps)
                )
            ):
            ...
            logger.debug(
                f"{iteration}: proposal REJECTED (old: {best_score}, new: {cur_score})"
            )
        ...
        if (
            ...
            and iteration - last_best_iteration >= stop_after_no_improvement
        ):
            logger.debug(
                f"{iteration}: no improvement for {stop_after_no_improvement} iterations, stopping"
            )
            break
    ...
    log_debug(
        logger,
        f"Done with mcmc iterations. Best score: {overall_best_score} at iteration {last_best_iteration} ({len(is_accepted_iteration)} accepted iterations)",
    )
    ...
    overall_best_diffs = create_diff_and_check_loh(
        ...
        skip_loh_check=skip_loh_check,
        use_cache=True,
        check_all_solutions=(has_loh and check_all_loh_solutions),
        single_time_limit=loh_time_limit,
    )
    ...
    assert best_sv_overlaps == (
        max(x for x in all_sv_overlaps if x is not None)
        if any(x is not None for x in all_sv_overlaps)
        else 0
    ), (best_sv_overlaps, np.max(all_sv_overlaps))
    ...
    ) or (not has_wgd and any([event[0] == event[1] for event in overall_best_events])):
        raise ValueError(
            "Invalid empty event (start == end) found. This is due to incorrect distance calculation"
        )
    ...
    if overall_best_score is None:
        logger.warning(
            f"Final best events for {cur_id} contain zero-width event; returning None"
        )
        return None
    ...
    if return_full:
        mcmc_result = Mcmc_result_full(
            *mcmc_result,
            sv_selected_events,
            all_events=all_best_events if has_wgd else np.array(all_best_events),
...
def get_event_dist_data_from_mcmc_proposal(
    ...
    if has_wgd:
        ...
        wgd_status = np.array(
            ["pre"] * len(event_proposal[0]) + ["post"] * len(event_proposal[1])
        )
    ...
    if not np.all(widths > 0):
        # Zero or negative widths arise when a proposal generates start_idx == end_idx
        # events that land on zero-gap segment boundaries (end[i] == start[i+1]), a
        # common artefact of 1bp gaps in the raw CNA data being collapsed by the
        # half-open interval convention used in centromere processing.
        bad_events = event_proposal[widths <= 0]
        raise ZeroWidthEventError(
            f"Zero/negative width events at zero-gap boundary: "
            f"events={bad_events.tolist()}, widths={widths[widths <= 0].tolist()}"
        )
    type = np.where(event_proposal[:, 0] < event_proposal[:, 1], "gain", "loss")

    (is_telomere_bound, is_whole_arm, is_whole_chrom) = (
        calc_telomere_bound_whole_arm_whole_chrom(
...
def create_mcmc_proposal(
    ...
    if has_wgd:
        return _create_mcmc_proposal_wgd(
            best_events,
            iteration,
            cn_profile,
            swap_event_based_on_score,
        ...
        return _create_mcmc_proposal_nowgd(
            best_events, iteration, swap_event_based_on_score, event_distances
        )


def _create_mcmc_proposal_wgd(
    cur_events,
    iteration,
    cn_profile,
    swap_event_based_on_score=True,
    ...
    while new_events is None:
        ...
        if random_var == 0:
            ...
            new_events = proposal_wgd_remove_bp(
                cur_events, cn_profile, total_cn=total_cn
            )
            cur_transition = "remove_bp"
        elif random_var == 2:
            new_events, option = proposal_wgd_extend_shorten_pre_gain(
                cur_events, cn_profile, return_selected_option=True, total_cn=total_cn
            )
            cur_transition = "extend_shorten_pre_gain-" + option
        elif random_var == 3:
            new_events, option = proposal_wgd_extend_shorten_pre_loss(
                cur_events, cn_profile, return_selected_option=True, total_cn=total_cn
            )
            cur_transition = "extend_shorten_pre_loss-" + option
        elif random_var == 4:
            new_events, option = proposal_wgd_switch_loh_loss(
                cur_events, cn_profile, return_pre_post=True
            )
            cur_transition = "switch_loh_loss-" + option
        elif (
            random_var >= 5
        ):  # this has double the probability because it's the fastest
            new_events = proposal_wgd_simple_swap(
                cur_events, swap_event_based_on_score, event_distances
            )
            cur_transition = "simple_swap"

        if n > 1_000:
            raise ValueError(
                f"No valid proposal found for current events: {cur_events}"
            )
...
def _create_mcmc_proposal_nowgd(
    best_events, iteration, swap_event_based_on_score=True, event_distances=None
):
    ...
    if (
        swap_event_based_on_score and iteration % 4 != 3 and event_distances is not None
    ):  # every 4th turn, choose completely random
...
def calc_event_distances_mcmc_wrapper(
    ...
    event_tuples_hash = (
        tuple(
            [
                tuple(sorted(pre_post)) if len(pre_post) > 0 else ()
                for pre_post in event_tuples
            ]
        )
    ...
    except ZeroWidthEventError as e:
        # Proposal has start==end event on a zero-gap boundary; skip this proposal.
        logger.debug(f"Skipping proposal with zero-width event: {e}")
        return None, None
...
def create_diff_and_check_loh(
    ...
    if has_wgd:
        event_proposal_ = (
            sorted((tuple(x) for x in event_proposal[0])),
            sorted([tuple(x) for x in event_proposal[1]]),
        )
...
@cache
def _create_diff_and_check_loh_impl(
    event_proposal,
    cn_profile,
    has_wgd=False,
    check_all_solutions=False,
    ...
    if has_wgd:
        ...
        diffs = get_events_diff_from_coords(
            ...
            cn_profile,
            lexsort_diffs=True,
            filter_missed_lohs=not skip_loh_check,
            fail_if_empty=False,
        )
    if len(diffs) == 0:
        return []
    ...
    if 0 in cn_profile and not skip_loh_check:
        ...
        if has_wgd:
            ...
            diffs_final = loh_filters_for_graph_result_diffs_wgd(
                diffs,
                cn_profile,
                return_all_solutions=check_all_solutions,
                shuffle_diffs=True,
                total_cn=total_cn,
            )
        else:
...
def run_multiple_mcmc(n_duplicates, **kwargs):
    ...
    for _duplicate in range(n_duplicates):
        (
            ...
            all_best_events,
            is_accepted_iteration,
        ) = mcmc_event_selection(**kwargs)
        duplication_score = np.min(np.array(all_scores)[is_accepted_iteration])
        duplication_sv_overlaps = np.max(
            np.array(all_sv_overlaps)[is_accepted_iteration]
        )
        all_duplications_scores.append(duplication_score)
        if (
            duplication_sv_overlaps >= best_duplication_sv_overlaps
            and duplication_score < best_duplicate_score
        ):
...
def plot_mcmc_scores(
    mcmc_result=None,
    all_scores=None,
    is_accepted_iteration=None,
    all_sv_overlaps=None,
    scores_full=None,
    sv_overlaps_full=None,
    ax=None,
    show_best_score=True,
    show_all_best_scores=True,
    title=None,
    show_svs=True,
):
    ...
    if mcmc_result is not None:
        ...
        if all_scores is None or is_accepted_iteration is None:
            raise ValueError(
                "either mcmc_result or all_scores and is_accepted_iteration must be provided"
            )
    if ax is None:
    ...
    if show_best_score:
        plt.axvline(
            np.array(is_accepted_iteration)[np.array(all_scores) == np.min(all_scores)][
                0
            ],
            color="red",
            label="best score",
    ...
    if show_all_best_scores:
        for x in np.array(is_accepted_iteration)[
            np.array(all_scores) == np.min(all_scores)
        ]:
            plt.axvline(x, color="grey", ls="--", alpha=0.5)

    if scores_full is not None:
        ax.plot(
            np.arange(len(scores_full))[np.array([x is not None for x in scores_full])],
            np.array(scores_full)[np.array([x is not None for x in scores_full])],
            "--",
            color="C0",
            alpha=0.25,
        )
    ...
    if all_sv_overlaps is not None and show_svs:
        ...
        if sv_overlaps_full is not None:
            ax2.plot(
                np.arange(len(sv_overlaps_full))[
                    np.array([x is not None for x in sv_overlaps_full])
                ],
                np.array(sv_overlaps_full)[
                    np.array([x is not None for x in sv_overlaps_full])
                ],
                "--",
                color="C1",
...
def create_best_events_df_from_mcmc(
    mcmc_result, chrom_segments, chrom_data=None, has_wgd=False
):
    ...
    if has_wgd:
        wgd_status = np.array(
            ["pre"] * len(best_diffs[0]) + ["post"] * len(best_diffs[1])
        )
        best_diffs = np.concatenate(
            [pre_post for pre_post in best_diffs if len(pre_post) > 0]
        )
    else:
    ...
    best_events_df = create_full_df_from_diff_df(
        best_events_df, cur_id, cur_chrom_segments
    )
    ...
    if chrom_data is not None:
        ...
        if (chrom_data.n_events - mcmc_result.sv_overlaps[-1]) <= 1:
            ...
            assert has_wgd or len(mcmc_result.sv_overlaps) == 1, (
                f"sv_overlaps: {mcmc_result.sv_overlaps}"
            )
...
def _create_proposal_from_cur_events(
    cur_events, remove_pre=None, add_pre=None, remove_post=None, add_post=None
):
    ...
    if remove_pre is not None:
        for event in remove_pre:
            event_proposal_pre = [
                event
                for event in event_proposal_pre
                if event not in remove_pre or remove_pre.remove(event)
            ]
    if remove_post is not None:
        for event in remove_post:
            event_proposal_post = [
                event
                for event in event_proposal_post
                if event not in remove_post or remove_post.remove(event)
            ]
...
def _removed_pre_gain_passes_loh_filter(
    cur_events, cur_pre_gain, cn_profile, which, new_pre_gain=None, total_cn=False
):
    ...
    if which == "remove_bp":
        ...
        raise ValueError(
            'Invalid parameter "which" for _removed_pre_gain_passes_loh_filter'
        )

    cur_diff_pre = get_events_diff_from_coords(
        [pre_events], cn_profile, lexsort_diffs=True, filter_missed_lohs=False
    )[0]
    cur_diff_pre = cur_diff_pre[:, start:end]
    if np.any(cur_diff_pre == -1):
        cur_diff_pre_loh_filtered = loh_filters_for_graph_result_diffs(
            [cur_diff_pre],
            cn_profile[start:end],
            total_cn=total_cn,
            single_time_limit=None,
...
def _added_pre_loss_passes_loh_filter(
    cur_events, cur_loss, cn_profile, which, new_pre_loss=None, total_cn=False
):
    ...
    if which == "add_bp":
        ...
        raise ValueError(
            'Invalid parameter "which" for _added_pre_loss_passes_loh_filter'
        )
    ...
    if np.logical_and(
        (cur_diff_pre != 1).all(axis=0), cn_profile[start:end] != 0
    ).any():
        return False
...
def proposal_wgd_simple_swap(
    cur_events, swap_event_based_on_score=False, event_distances=None
):
    ...
    if swap_event_based_on_score and event_distances is not None:
        ...
        p = [
            np.ones(len(cur_events[0])) / len(cur_events[0]),
            np.ones(len(cur_events[1])) / len(cur_events[1]),
        ]

    pre_post = [
        label for label, events in zip(["pre", "post"], cur_events) if len(events) >= 2
    ]
    if len(pre_post) == 0:
        return None
    ...
    ind1, ind2 = np.random.choice(
        range(len(new_events[pre_post_ind])), size=2, p=p[pre_post_ind]
    )
    cur = (new_events[pre_post_ind][ind1][0], new_events[pre_post_ind][ind2][1])
    new_events[pre_post_ind][ind2] = (
        new_events[pre_post_ind][ind2][0],
        new_events[pre_post_ind][ind1][1],
    )
    new_events[pre_post_ind][ind1] = cur

    if any(
        [event[0] == event[1] for event in new_events[0]]
        + [event[0] == event[1] for event in new_events[1]]
    ):
        return None

    # no change
    if sorted(
        (cur_events[pre_post_ind][ind1], cur_events[pre_post_ind][ind2])
...
def proposal_wgd_add_bp(cur_events, cn_profile, total_cn=False):
    ...
    doubled_starts = [
        (e, "start")
        for e, c in Counter([event[0] for event in cur_events[1]]).items()
        if c >= 2
    ]
    doubled_ends = [
        (e, "end")
        for e, c in Counter([event[1] for event in cur_events[1]]).items()
        if c >= 2
    ]
    ...
    cur_events_with_bp = [
        event for event in cur_events[1] if event[start_end_i] == cur_bp
    ]
    ...
    if cur_event_to_double[0] > cur_event_to_double[1]:
        if not _added_pre_loss_passes_loh_filter(
            cur_events,
            cur_event_to_double,
            cn_profile,
            which="add_bp",
    ...
    event_proposal = _create_proposal_from_cur_events(
        cur_events,
        add_pre=[cur_event_to_double],
        remove_post=[cur_event_to_double, other_event_with_bp],
        add_post=[
            (cur_event_to_double[1], other_event_with_bp[1])
            if start_end == "start"
            else (other_event_with_bp[0], cur_event_to_double[0])
        ],
    )
...
def proposal_wgd_remove_bp(cur_events, cn_profile, total_cn=False):
    ...
    cur_pre_gains_start = [
        (event, "gain", "start")
        for event in cur_events[0]
        if event[0] in doubled_bps and event[0] < event[1]
    ]
    cur_pre_gains_end = [
        (event, "gain", "end")
        for event in cur_events[0]
        if event[1] in doubled_bps and event[0] < event[1]
    ]
    cur_pre_losses_start = [
        (event, "loss", "start")
        for event in cur_events[0]
        if event[0] in doubled_bps and event[0] > event[1]
    ]
    ...
    if (
        len(cur_pre_gains_start)
        + len(cur_pre_gains_end)
        + len(cur_pre_losses_start)
        + len(cur_pre_losses_end)
        == 0
    ):
        return None
    ...
    if gain_loss == "gain" and not _removed_pre_gain_passes_loh_filter(
        cur_events, cur_pre_event, cn_profile, which="remove_bp", total_cn=total_cn
    ):
        return None

    # find a post event that ends/starts at doubled bp
    cur_post_events = [
        event for event in cur_events[1] if event[start_end_j] == cur_doubled_bp
    ]
    ...
    event_proposal = _create_proposal_from_cur_events(
        cur_events,
        remove_pre=[cur_pre_event],
        remove_post=[cur_post_event],
        add_post=[
            cur_pre_event,
            (cur_pre_event[0], cur_post_event[1])
            if start_end == "end"
            else (cur_post_event[0], cur_pre_event[1]),
        ],
    )
    return event_proposal


def proposal_wgd_extend_shorten_pre_gain(
    cur_events, cn_profile, return_selected_option=False, total_cn=False
):
    ...
    # for A/B
    all_post_gains_start_at_pre_end = [
        event for event in all_post_gains if event[0] == cur_pre_gain[1]
    ]
    all_post_gains_end_at_pre_start = [
        event for event in all_post_gains if event[1] == cur_pre_gain[0]
    ]
    all_post_losses_start_at_pre_end_and_inside_pre = [
        event
        for event in all_post_losses
        if event[0] == cur_pre_gain[1] and event[1] > cur_pre_gain[0]
    ]
    all_post_losses_end_at_pre_start_and_inside_pre = [
        event
        for event in all_post_losses
        if event[1] == cur_pre_gain[0] and event[0] < cur_pre_gain[1]
    ]
    # for C/D
    all_post_losses_start_at_pre_end = [
        event for event in all_post_losses if event[0] == cur_pre_gain[1]
    ]
    all_post_losses_end_at_pre_start = [
        event for event in all_post_losses if event[1] == cur_pre_gain[0]
    ]
    all_post_losses_right = [
        event for event in all_post_losses if event[1] > cur_pre_gain[1]
    ]
    all_post_losses_left = [
        event for event in all_post_losses if event[0] < cur_pre_gain[0]
    ]
    all_post_losses_right_partly_inside_pre = [
        event
        for event in all_post_losses
        if event[1] < cur_pre_gain[1]
        and event[1] > cur_pre_gain[0]
        and event[0] > cur_pre_gain[1]
    ]
    all_post_losses_left_partly_inside_pre = [
        event
        for event in all_post_losses
        if event[0] > cur_pre_gain[0]
        and event[0] < cur_pre_gain[1]
        and event[1] < cur_pre_gain[0]
    ]
    # for E/F
    all_post_events_start_left = [
        event for event in cur_events[1] if event[0] < cur_pre_gain[0]
    ]
    all_post_events_end_right = [
        event for event in cur_events[1] if event[1] > cur_pre_gain[1]
    ]
    all_post_events_start_right_and_inside = [
        event
        for event in cur_events[1]
        if event[0] > cur_pre_gain[0] and event[0] < cur_pre_gain[1]
    ]
    all_post_events_end_left_and_inside = [
        event
        for event in cur_events[1]
        if event[1] < cur_pre_gain[1] and event[1] > cur_pre_gain[0]
    ]
    all_post_events_end_right_same_end = _select_events_with_same_start_or_end(
        all_post_events_end_right, start_end="end"
    )
    all_post_events_start_left_same_start = _select_events_with_same_start_or_end(
        all_post_events_start_left, start_end="start"
    )
    all_post_events_start_right_and_inside_same_start = (
        _select_events_with_same_start_or_end(
            all_post_events_start_right_and_inside, start_end="start"
        )
    )
    all_post_events_end_left_and_inside_same_end = (
        _select_events_with_same_start_or_end(
            all_post_events_end_left_and_inside, start_end="end"
        )
    )
    ...
    options = [
        (
            "A",
            len(all_post_gains_start_at_pre_end) > 0
            or len(all_post_gains_end_at_pre_start) > 0,
        ),
        (
            "B",
            len(all_post_losses_start_at_pre_end_and_inside_pre) > 0
            or len(all_post_losses_end_at_pre_start_and_inside_pre) > 0,
        ),
        (
            "C",
            (
                (
                    len(all_post_losses_start_at_pre_end) > 0
                    and len(all_post_losses_right) > 0
                )
                or (
                    len(all_post_losses_end_at_pre_start) > 0
                    and len(all_post_losses_left) > 0
                )
            ),
        ),
        (
            ...
            (
                len(all_post_losses_start_at_pre_end) > 0
                and len(all_post_losses_right_partly_inside_pre) > 0
            )
            or (
                len(all_post_losses_end_at_pre_start) > 0
                and len(all_post_losses_left_partly_inside_pre) > 0
            ),
        ),
        (
            "E",
            len(all_post_events_end_right_same_end) > 0
            or len(all_post_events_start_left_same_start) > 0,
        ),
        (
            "F",
            len(all_post_events_start_right_and_inside_same_start) > 0
            or len(all_post_events_end_left_and_inside_same_end) > 0,
        ),
    ]
    ...
    if selected_option == "A":
        cur_post_gain, pre_start_end = random.choice(
            [(x, "end") for x in all_post_gains_start_at_pre_end]
            + [(x, "start") for x in all_post_gains_end_at_pre_start]
        )

        event_proposal = _create_proposal_from_cur_events(
            cur_events,
            remove_pre=[cur_pre_gain],
            add_pre=[
                (cur_post_gain[0], cur_pre_gain[1])
                if pre_start_end == "start"
                else (cur_pre_gain[0], cur_post_gain[1])
            ],
            remove_post=[cur_post_gain],
            add_post=[(cur_post_gain[1], cur_post_gain[0])],
        )

    # OPTION B: shorten pre-gain and flip a loss (turn to gain) that ends at doubled bp but starts after pre-gain starts
    elif selected_option == "B":
        cur_post_loss, pre_start_end = random.choice(
            [(x, "start") for x in all_post_losses_end_at_pre_start_and_inside_pre]
            + [(x, "end") for x in all_post_losses_start_at_pre_end_and_inside_pre]
        )
        new_pre_gain = (
            (cur_post_loss[0], cur_pre_gain[1])
            if pre_start_end == "start"
            else (cur_pre_gain[0], cur_post_loss[1])
        )

        if not _removed_pre_gain_passes_loh_filter(
            cur_events,
            cur_pre_gain,
            cn_profile,
            which="shorten_pre_gain",
            new_pre_gain=new_pre_gain,
            total_cn=total_cn,
        ):
            if return_selected_option:
        ...
        event_proposal = _create_proposal_from_cur_events(
            cur_events,
            remove_pre=[cur_pre_gain],
            add_pre=[new_pre_gain],
            remove_post=[cur_post_loss],
            add_post=[(cur_post_loss[1], cur_post_loss[0])],
        )

    # OPTION C: extend pre-gain and extend two losses inwards from either side
    elif selected_option == "C":
        pre_start_end_prob = np.array(
            [
                min(len(all_post_losses_end_at_pre_start), len(all_post_losses_left)),
                min(len(all_post_losses_start_at_pre_end), len(all_post_losses_right)),
            ]
        )
        pre_start_end = np.random.choice(
            ["start", "end"], p=pre_start_end_prob / np.sum(pre_start_end_prob)
        )
        ...
        event_proposal = _create_proposal_from_cur_events(
            ...
            add_post=[
                (cur_post_loss_right[1], cur_post_loss_left[1]),
                (cur_post_loss_right[0], cur_post_loss_left[0]),
            ],
        )
    ...
    # OPTION D: shorten pre-gain and shorten two losses outwards (opposite of C)
        ...
        pre_start_end_prob = np.array(
            [
                min(
                    len(all_post_losses_end_at_pre_start),
                    len(all_post_losses_left_partly_inside_pre),
                ),
                min(
                    len(all_post_losses_start_at_pre_end),
                    len(all_post_losses_right_partly_inside_pre),
                ),
            ]
        )
        pre_start_end = np.random.choice(
    ...
    # OPTION E: extend pre-gain and change two events with the same start to the left/right from right/left side (gains will turn to losses)
        ...
        assert (
            left_right == "right" and cur_post_events[0][1] == cur_post_events[1][1]
        ) or (left_right == "left" and cur_post_events[0][0] == cur_post_events[1][0])

        event_proposal = _create_proposal_from_cur_events(
            cur_events,
            remove_pre=[cur_pre_gain],
            remove_post=cur_post_events,
            add_pre=[
                (cur_pre_gain[0], cur_post_events[0][1])
                if left_right == "right"
                else (cur_post_events[0][0], cur_pre_gain[1])
            ],
            add_post=(
                [
                    (cur_post_events[0][0], cur_pre_gain[1]),
                    (cur_post_events[1][0], cur_pre_gain[1]),
                ]
                if left_right == "right"
                else [
                    (cur_pre_gain[0], cur_post_events[0][1]),
                    (cur_pre_gain[0], cur_post_events[1][1]),
                ]
            ),
        )
    ...
    # OPTION F: shorten pre-gain and shorten two losses with the same end to the right from left side (opposite of E)
        ...
        selected_group, left_right = random.choice(
            ...
            + [
                (x, "right")
                for x in all_post_events_start_right_and_inside_same_start.values()
            ]
        )
        cur_post_losses = random.sample(selected_group, 2)
        assert (
            left_right == "right" and cur_post_losses[0][0] == cur_post_losses[1][0]
        ) or (left_right == "left" and cur_post_losses[0][1] == cur_post_losses[1][1])
        new_pre_gain = (
            (cur_post_losses[0][0], cur_pre_gain[1])
            if left_right == "right"
            else (cur_pre_gain[0], cur_post_losses[0][1])
        )
        ...
        event_proposal = _create_proposal_from_cur_events(
            cur_events,
            remove_pre=[cur_pre_gain],
            remove_post=cur_post_losses,
            add_pre=[new_pre_gain],
            add_post=(
                [
                    (cur_pre_gain[0], cur_post_losses[0][1]),
                    (cur_pre_gain[0], cur_post_losses[1][1]),
                ]
                if left_right == "right"
                else [
                    (cur_post_losses[0][0], cur_pre_gain[1]),
                    (cur_post_losses[1][0], cur_pre_gain[1]),
                ]
            ),
        )
    else:
    ...
    assert len(event_proposal[0]) + len(event_proposal[1]) == len(cur_events[0]) + len(
        cur_events[1]
    )
    if return_selected_option:
...
def proposal_wgd_extend_shorten_pre_loss(
    cur_events, cn_profile, return_selected_option=False, total_cn=False
):
    ...
    # for A/B
    all_post_losses_start_at_pre_end = [
        event for event in all_post_losses if event[0] == cur_pre_loss[1]
    ]
    all_post_losses_end_at_pre_start = [
        event for event in all_post_losses if event[1] == cur_pre_loss[0]
    ]
    all_post_gains_start_at_pre_end_and_inside_pre = [
        event
        for event in all_post_gains
        if event[0] == cur_pre_loss[1] and event[1] < cur_pre_loss[0]
    ]
    all_post_gains_end_at_pre_start_and_inside_pre = [
        event
        for event in all_post_gains
        if event[1] == cur_pre_loss[0] and event[0] > cur_pre_loss[1]
    ]
    # for C/D
    all_post_gains_start_at_pre_end = [
        event for event in all_post_gains if event[0] == cur_pre_loss[1]
    ]
    all_post_gains_end_at_pre_start = [
        event for event in all_post_gains if event[1] == cur_pre_loss[0]
    ]
    all_post_gains_right = [
        event for event in all_post_gains if event[0] > cur_pre_loss[0]
    ]
    all_post_gains_left = [
        event for event in all_post_gains if event[1] < cur_pre_loss[1]
    ]
    all_post_gains_left_partly_inside_pre = [
        event
        for event in all_post_gains
        if event[1] > cur_pre_loss[1]
        and event[1] < cur_pre_loss[0]
        and event[0] < cur_pre_loss[1]
    ]
    all_post_gains_right_partly_inside_pre = [
        event
        for event in all_post_gains
        if event[0] < cur_pre_loss[0]
        and event[0] > cur_pre_loss[1]
        and event[1] > cur_pre_loss[0]
    ]
    # for E/F
    all_post_events_start_right = [
        event for event in cur_events[1] if event[0] > cur_pre_loss[0]
    ]
    all_post_events_end_left = [
        event for event in cur_events[1] if event[1] < cur_pre_loss[1]
    ]
    all_post_events_start_right_same_start = _select_events_with_same_start_or_end(
        all_post_events_start_right, start_end="start"
    )
    all_post_events_end_left_same_end = _select_events_with_same_start_or_end(
        all_post_events_end_left, start_end="end"
    )
    all_post_events_start_left_and_inside = [
        event
        for event in cur_events[1]
        if event[0] > cur_pre_loss[1] and event[0] < cur_pre_loss[0]
    ]
    all_post_events_end_right_and_inside = [
        event
        for event in cur_events[1]
        if event[1] > cur_pre_loss[1] and event[1] < cur_pre_loss[0]
    ]
    all_post_events_start_left_and_inside_same_start = (
        _select_events_with_same_start_or_end(
            all_post_events_start_left_and_inside, start_end="start"
        )
    )
    all_post_events_end_right_and_inside_same_end = (
        _select_events_with_same_start_or_end(
            all_post_events_end_right_and_inside, start_end="end"
        )
    )
    ...
    options = [
        (
            "A",
            len(all_post_losses_start_at_pre_end) > 0
            or len(all_post_losses_end_at_pre_start) > 0,
        ),
        (
            "B",
            len(all_post_gains_start_at_pre_end_and_inside_pre) > 0
            or len(all_post_gains_end_at_pre_start_and_inside_pre) > 0,
        ),
        (
            "C",
            (
                (
                    len(all_post_gains_start_at_pre_end) > 0
                    and len(all_post_gains_left) > 0
                )
                or (
                    len(all_post_gains_end_at_pre_start) > 0
                    and len(all_post_gains_right) > 0
                )
            ),
        ),
        (
            ...
            (
                len(all_post_gains_start_at_pre_end) > 0
                and len(all_post_gains_left_partly_inside_pre) > 0
            )
            or (
                len(all_post_gains_end_at_pre_start) > 0
                and len(all_post_gains_right_partly_inside_pre) > 0
            ),
        ),
        (
            "E",
            len(all_post_events_start_right_same_start) > 0
            or len(all_post_events_end_left_same_end) > 0,
        ),
        (
            "F",
            len(all_post_events_start_left_and_inside_same_start) > 0
            or len(all_post_events_end_right_and_inside_same_end) > 0,
        ),
    ]
    ...
    if selected_option == "A":
        cur_post_loss, pre_start_end = random.choice(
            [(x, "end") for x in all_post_losses_start_at_pre_end]
            + [(x, "start") for x in all_post_losses_end_at_pre_start]
        )
        new_pre_loss = (
            (cur_post_loss[0], cur_pre_loss[1])
            if pre_start_end == "start"
            else (cur_pre_loss[0], cur_post_loss[1])
        )
        if not _added_pre_loss_passes_loh_filter(
            cur_events,
            cur_pre_loss,
            cn_profile,
            which="extend_pre_loss",
            new_pre_loss=new_pre_loss,
            total_cn=total_cn,
        ):
            if return_selected_option:
                return None, "A"
    ...
    # OPTION B: shorten pre-loss and flip a gain (turn to loss) that ends at doubled bp but starts after pre-loss starts
        ...
        cur_post_gain, pre_start_end = random.choice(
            [(x, "start") for x in all_post_gains_end_at_pre_start_and_inside_pre]
            + [(x, "end") for x in all_post_gains_start_at_pre_end_and_inside_pre]
        )
        new_pre_loss = (
            (cur_post_gain[0], cur_pre_loss[1])
            if pre_start_end == "start"
            else (cur_pre_loss[0], cur_post_gain[1])
        )
    ...
    # OPTION C: extend pre-loss and extend two gains inwards from either side
        ...
        pre_start_end_prob = np.array(
            [
                min(len(all_post_gains_end_at_pre_start), len(all_post_gains_right)),
                min(len(all_post_gains_start_at_pre_end), len(all_post_gains_left)),
            ]
        )
        pre_start_end = np.random.choice(
            ["pre-start", "pre-end"], p=pre_start_end_prob / np.sum(pre_start_end_prob)
        )
        ...
        event_proposal = _create_proposal_from_cur_events(
            ...
            add_post=[
                (cur_post_gain_left[0], cur_post_gain_right[0]),
                (cur_post_gain_left[1], cur_post_gain_right[1]),
            ],
        )
    ...
    # OPTION D: shorten pre-loss and shorten two gains outwards (opposite of C)
        ...
        pre_start_end_prob = np.array(
            [
                min(
                    len(all_post_gains_end_at_pre_start),
                    len(all_post_gains_right_partly_inside_pre),
                ),
                min(
                    len(all_post_gains_start_at_pre_end),
                    len(all_post_gains_left_partly_inside_pre),
                ),
            ]
        )
        pre_start_end = np.random.choice(
    ...
    # OPTION E: extend pre-loss and change two events with the same start to the left/right from right/left side (losses will turn to gains)
        ...
        cur_post_events = random.sample(selected_group, 2)
        assert (
            left_right == "right" and cur_post_events[0][0] == cur_post_events[1][0]
        ) or (left_right == "left" and cur_post_events[0][1] == cur_post_events[1][1])
        new_pre_loss = (
            (cur_post_events[0][0], cur_pre_loss[1])
            if left_right == "right"
            else (cur_pre_loss[0], cur_post_events[0][1])
        )
        ...
        event_proposal = _create_proposal_from_cur_events(
            ...
            add_post=(
                [
                    (cur_pre_loss[0], cur_post_events[0][1]),
                    (cur_pre_loss[0], cur_post_events[1][1]),
                ]
                if left_right == "right"
                else [
                    (cur_post_events[0][0], cur_pre_loss[1]),
                    (cur_post_events[1][0], cur_pre_loss[1]),
                ]
            ),
        )
    ...
    # OPTION F: shorten pre-loss and shorten two gains with the same end to the right from left side (opposite of E)
        ...
        selected_group, left_right = random.choice(
            [
                (x, "left")
                for x in all_post_events_start_left_and_inside_same_start.values()
            ]
            + [
                (x, "right")
                for x in all_post_events_end_right_and_inside_same_end.values()
            ]
        )
        cur_post_gains = random.sample(selected_group, 2)
        assert (
            left_right == "right" and cur_post_gains[0][1] == cur_post_gains[1][1]
        ) or (left_right == "left" and cur_post_gains[0][0] == cur_post_gains[1][0])

        event_proposal = _create_proposal_from_cur_events(
            cur_events,
            remove_pre=[cur_pre_loss],
            remove_post=cur_post_gains,
            add_pre=[
                (cur_pre_loss[0], cur_post_gains[0][1])
                if left_right == "right"
                else (cur_post_gains[0][0], cur_pre_loss[1])
            ],
            add_post=(
                [
                    (cur_post_gains[0][0], cur_pre_loss[1]),
                    (cur_post_gains[1][0], cur_pre_loss[1]),
                ]
                if left_right == "right"
                else [
                    (cur_pre_loss[0], cur_post_gains[0][1]),
                    (cur_pre_loss[0], cur_post_gains[1][1]),
                ]
            ),
        )
    else:
...
def _proposal_is_unchanged(event_proposal, best_events, has_wgd):
    if has_wgd:
        if len(event_proposal[0]) != len(best_events[0]) or len(
            event_proposal[1]
        ) != len(best_events[1]):
            return False
    ...
    if has_wgd:
        ...
        overall_best_events = np.stack(
            [
                ...
                overall_best_diffs.shape[1]
                - np.argmax(np.abs(overall_best_diffs)[:, ::-1], axis=1),
            ]
        ).T
        overall_best_events[overall_best_diffs.min(axis=1) == -1] = overall_best_events[
            overall_best_diffs.min(axis=1) == -1
        ][:, ::-1]
