...
def full_paths_from_graph_with_sv(
    cur_id,
    is_wgd,
    sv_data,
    chrom_segments,
    ...
    use_cache=True,
    skip_loh_checks=False,
    all_loh_solutions=True,
    **kwargs,
):
    ...
    log_debug(
        logger,
        f"Creating full paths ({'without' if sv_data is None else 'with'} SV overlaps) for {cur_id} {'WGD' if is_wgd else 'noWGD'}. Nr events: {chrom.n_events}. CN profile: {cn_profile}",
    )
    ...
    unique_events = {
        i: d for i, d in enumerate(set(item for sublist in diffs for item in sublist))
    }
    ...
    unique_solutions = [
        Counter({k: v for k, v in x}) for x in {frozenset(c.items()) for c in solutions}
    ]
    log_debug(logger, f"Found {len(unique_events)} unique events")
    log_debug(
        logger,
        f"Found {len(solutions)} solutions of which {len(unique_solutions)} are unique",
    )
    assert all(
        [solution.total() == (chrom.n_events) for solution in unique_solutions]
    ), (
        f"expected nr of events: {chrom.n_events}. nr of events per solution: {[solution.total() for solution in unique_solutions]}"
    )

    if any([event.diff.find("1") == -1 for event in unique_events.values()]):
        raise ValueError(
            "Invalid empty events found. This usually means that the number of events "
            "was calculated incorrectly for WGD samples."
        )

    if len(unique_solutions) == 1:
        if (
            sv_selected_events is not None
            and (chrom.n_events - len(sv_selected_events)) <= 1
            and chrom.n_events > 1
        ):
            solved = "sv"
        else:
            solved = "unamb"
...
def _full_paths_implementation_nowgd(
    ...
    if len(starts) > 0:
        ...
        assert [len(p) == len(paths[0]) for p in paths], (
            f"Paths have different number of events: {[len(p) for p in paths]}"
        )
        assert len(paths[0]) + (
            0 if sv_selected_events is None else len(sv_selected_events)
        ) == len(starts_), (
            f"Number of paths do not match: {len(paths[0])} + {0 if sv_selected_events is None else len(sv_selected_events)} vs {len(starts_)}"
        )
    else:
        paths = []
        assert sv_selected_events is not None, (
            f"len(start) == 0 but no sv_selected_events. starts = {starts}"
        )
        assert len(sv_selected_events) == len(starts_), (
            f"Number of SV selected events do not match: {len(sv_selected_events)} vs {len(starts_)}"
        )

    if paths is None:
        raise NotImplementedError("No solutions (paths) found")

    diffs_ = diffs = get_events_diff_from_coords(
        paths,
        cn_profile,
        lexsort_diffs=True,
    ...
    if sv_selected_events is not None:
        diffs_sv_solved = get_events_diff_from_coords(
            [sv_selected_events],
            cn_profile,
            lexsort_diffs=True,
            filter_missed_lohs=False,
    ...
    assert [len(d) == len(diffs[0]) for d in diffs], (
        f"Diffs have different number of events: {[len(d) for d in diffs]}"
    )
    assert len(diffs[0]) == len(starts_), (
        f"Number of diffs do not match: {len(diffs[0])} ({len(diffs_[0])} + {len(diffs_sv_solved) if diffs_sv_solved is not None else 0}) vs {len(starts_)}"
    )

    # <= -1 is needed because for WGD it can be -2
    diffs = [
        diff for diff in diffs if (diff[:, cn_profile == 0] <= -1).any(axis=0).all()
    ]
...
def _full_paths_from_graph_with_sv_nowgd(
    ...
    starts_, ends_ = get_starts_and_ends(
    ...
    log_debug(
        logger,
        f"Current ID has {len(starts_)} events and {len(cur_sv_data) if cur_sv_data is not None else 0} SVs",
    )
    if cur_sv_data is not None:
        ...
        log_debug(
            logger,
            f"After SV matching, {len(sv_selected_events) if sv_selected_events is not None else 0} events are selected. {len(starts)} events remain",
        )
        log_debug(
            logger,
            f"Selected SV events: {sv_selected_events if sv_selected_events is not None else 'None'}",
        )
        assert len(starts) == len(ends), (
            f"Number of starts and ends do not match: {len(starts)} vs {len(ends)}"
        )
        assert sv_selected_events is None or len(sv_selected_events) + len(
            starts
        ) == len(starts_), (
            f"Number of SV selected events do not match: {len(sv_selected_events)} + {len(starts)} vs {len(starts_)}"
        )
    else:
        starts = starts_
        ends = ends_
    ...
    if len(diffs) == 0 and sv_selected_events is not None:
        logger.warning(
            f"No viable solutions (diffs) found with SVs. Trying without SVs"
        )
        sv_selected_events = None
        diffs = _full_paths_implementation_nowgd(
    ...
    if not skip_loh_checks and 0 in cn_profile:
        diffs = loh_filters_for_graph_result_diffs(
            ...
            cn_profile,
            total_cn=total_cn,
            return_all_solutions=all_loh_solutions,
            shuffle_diffs=True,
        )
    ...
    diffs = [
        [
            Diff(diff="".join(map(str, np.abs(x))), is_gain=x.max() == 1, wgd="nowgd")
            for x in diff
        ]
        for diff in diffs
    ]
...
def _full_paths_from_graph_with_sv_wgd(
    ...
    **kwargs,
    ...
    if cur_sv_data is not None and cur_chrom_segments is not None:
        ...
        log_debug(
            logger,
            f"SVs selected {len(sv_selected_paths)} paths{'. not using SVs here' if len(sv_selected_paths) == 0 else ''}",
        )
    ...
    if not skip_loh_checks and 0 in cn_profile:
        log_debug(
            logger,
            f"Performing LOH filters (returning {'all solutions' if all_loh_solutions else 'a single solution'})",
        )
    ...
    diffs = [
        [
            Diff(diff="".join(map(str, np.abs(x))), is_gain=x.max() == 1, wgd="pre")
            for x in diff[0]
        ]
        + [
            Diff(diff="".join(map(str, np.abs(x))), is_gain=x.max() == 1, wgd="post")
            for x in diff[1]
        ]
        for diff in diffs
    ]
...
def raw_events_from_FullPaths(full_paths, wgd=False):
    full_path_events = [
        [full_paths.events[x] for x in list(sol.elements())]
        for sol in full_paths.solutions
    ]
    if wgd:
        full_path_events = [
            [
                [
                    (x.diff.find("1"), (x.diff.rfind("1") + 1))
                    if x.is_gain
                    else (x.diff.rfind("1") + 1, x.diff.find("1"))
                    for x in sol
                    if x.wgd == "pre"
                ],
                [
...
def connect_start_ends_using_svs(
    cur_chrom_segments,
    cur_sv_data,
    starts,
    ends,
    ...
    cur_segment_breakpoints_starts = np.append(
        cur_chrom_segments["start"].values, cur_chrom_segments["end"].values[-1]
    )
    cur_segment_breakpoints_ends = np.append(
        cur_chrom_segments["start"].values[0], cur_chrom_segments["end"].values
    )

    if (
        "start_for_overlap" not in cur_sv_data.columns
        or "end_for_overlap" not in cur_sv_data.columns
    ):
        cur_sv_data["start_for_overlap"] = cur_sv_data["start"]
        cur_sv_data["end_for_overlap"] = cur_sv_data["end"]

        cur_sv_data.loc[cur_sv_data["svclass"] == "DEL", "start_for_overlap"] = (
            cur_sv_data.loc[cur_sv_data["svclass"] == "DEL", "end"]
        )
        cur_sv_data.loc[cur_sv_data["svclass"] == "DEL", "end_for_overlap"] = (
            cur_sv_data.loc[cur_sv_data["svclass"] == "DEL", "start"]
        )

    sv_start_matches = (
        np.abs(
            cur_segment_breakpoints_starts[unique_starts][:, None]
            - cur_sv_data["start_for_overlap"].values
        )
        < sv_matching_threshold
    )
    sv_end_matches = (
        np.abs(
            cur_segment_breakpoints_ends[unique_ends][:, None]
            - cur_sv_data["end_for_overlap"].values
        )
        < sv_matching_threshold
    )
    # invalid_svs = (sv_start_matches.sum(axis=0) == 0) | (sv_end_matches.sum(axis=0) == 0)
    valid_svs = np.logical_and(
        (sv_start_matches.sum(axis=0) == 1), (sv_end_matches.sum(axis=0) == 1)
    )
    ...
    if valid_svs.any():
        ...
        assert (
            sv_selected_events.shape[0] == valid_svs.sum()
            and sv_selected_events.shape[1] == 2
        ), f"sv_selected_events: {sv_selected_events.shape}"
        correct_gain_loss = (np.diff(sv_selected_events, axis=1) > 0)[:, 0] == (
            cur_sv_data["svclass"][valid_svs].values == "DUP"
        )
        sv_selected_events = sv_selected_events[correct_gain_loss]
        starts_ = np.array(
            list((Counter(starts) - Counter(sv_selected_events[:, 0])).elements())
        ).copy()
        ends_ = np.array(
            list((Counter(ends) - Counter(sv_selected_events[:, 1])).elements())
        ).copy()
    else:
        sv_selected_events = None
        starts_ = starts.copy()
...
def get_starts_and_ends_for_wgd(cn_profile, adjust_loh_wgd=True, total_cn=False):
    if adjust_loh_wgd and 0 in cn_profile:
        profile_adjust_nowgd = adjust_profile_for_loh(
            cn_profile, wgd=False, total_cn=total_cn
        )
        profile_adjust_wgd = adjust_profile_for_loh(
            cn_profile, wgd=True, total_cn=total_cn
        )
        ...
        loh_combinations = sum(
            [
                list(itertools.combinations(differing_loh_adjust, i + 1))
                for i in range(len(differing_loh_adjust))
            ],
            [],
        )
...
def get_events_from_graph_wgd(
    ...
    n_events = max(
        ...
        int(
            float(
                fstlib.score(
                    T_forced_WGD,
                    diploid_fsa_total_cn if total_cn else diploid_fsa,
                    fsa_from_string("".join(cn_profile.astype(str))),
                )
            )
        )
        - 1,
    ...
    log_debug(
        logger,
        f"{'LOH in profile' if 0 in cn_profile else 'No LOH in profile'}. Initial starts: {all_starts[0]} and ends: {all_ends[0]}. Running {len(all_starts)} different LOH adjustments\n",
    )

    for starts, ends in zip(all_starts, all_ends):
        log_debug(
            logger,
            f"Current LOH adjustment: {len(starts) - len(all_starts[0])} LOH adj added. Added starts: {np.sort(list((Counter(starts) - Counter(all_starts[0])).elements()))} and ends: {np.sort(list((Counter(ends) - Counter(all_ends[0])).elements()))}",
        )
    ...
    log_debug(
        logger,
        f"Found {len(all_paths)} paths for all {len(all_starts)} LOH adjustments",
    )

    if remove_duplicates:
        # remove duplicate paths
        paths_ = list(
            set([(tuple(sorted(x[0])), tuple(sorted(x[1]))) for x in all_paths])
        )
        ...
        assert len(paths_) == len(all_paths), (
            f"found non-unique paths ({len(paths_)} vs {len(all_paths)}): {[k for k, v in Counter([(tuple(sorted(x[0])), tuple(sorted(x[1]))) for x in all_paths]).items() if v > 1]}"
        )
    ...
    if assert_correct:
        ...
        assert all([len(x[0]) + len(x[1]) == n_events for x in all_paths]), (
            f"not all paths have length {n_events}"
        )
        ...
        diffs = get_events_diff_from_coords(
            paths_, cn_profile, filter_missed_lohs=False, fail_if_empty=False
        )
        diffs_sum = np.clip(
            [np.sum(d, axis=0) + (4 if total_cn else 2) for d in diffs],
            a_min=0,
            a_max=None,
        )
        assert all([np.all((d == cn_profile)[cn_profile != 0]) for d in diffs_sum]), (
            f"not all diffs create the correct profile"
        )
...
def get_events_from_graph_step(
    starts, ends, use_cache=False, time_limit=None, path_limit=None
):
    if len(starts) == 0:
        return []
    ...
    if use_cache:
        ...
        return _get_events_from_graph_step(
            starts, list(ends), time_limit=time_limit, path_limit=path_limit
        )[0]
...
def _get_events_from_graph_step_cache(starts, ends):
    ...
    res = []
    for chosen_ends in set(
        itertools.combinations([e for e in ends if e != s[0]], s[1])
    ):
        new_ends = ends.copy()
...
def _get_events_from_graph_step(
    starts,
    ends,
    i=0,
    res=None,
...
def adjust_profile_for_loh(profile, wgd=False, total_cn=False):
    ...
    loh_adjust = (
        np.stack(
            [np.roll(profile, 1)[loh_pos], np.roll(profile, -1)[loh_pos]], axis=1
        ).min(axis=1)
        - pad_value
    )
...
def get_starts_and_ends(
    profile, prior_profile=None, loh_adjust=True, wgd=False, total_cn=False
):
...
def create_random_start_end_pairs(starts, ends, n_paths, pre_selected_events=None):
    if pre_selected_events is not None:
        ...
        pre_selected_starts, pre_selected_ends = np.array(
            list(zip(*pre_selected_events))
        )
        vals, counts = np.unique(pre_selected_starts, return_counts=True)
        for val, count in zip(vals, counts):
            starts = np.delete(starts, np.where(starts == val)[0][:count])
        assert len(starts) == n_events - len(pre_selected_starts), (
            f"pre_selected_starts not in starts: {pre_selected_starts}"
        )

        vals, counts = np.unique(pre_selected_ends, return_counts=True)
        for val, count in zip(vals, counts):
            ends = np.delete(ends, np.where(ends == val)[0][:count])
        assert len(ends) == n_events - len(pre_selected_ends), (
            f"pre_selected_ends not in ends: {pre_selected_ends}"
        )
    else:
        pre_selected_events = []
    ...
    events = [
        pre_selected_events + [(s, e) for s, e in zip(starts, cur_ends)]
        for cur_ends in random_ends
    ]

    return events


def get_events_diff_from_coords(
    event_coords,
    profile,
    lexsort_diffs=False,
    filter_missed_lohs=True,
    fail_if_empty=True,
):
    """
...
def get_events_diff_from_coords_wgd(
    event_coords,
    profile,
    lexsort_diffs=True,
    filter_missed_lohs=False,
    ...
    diffs_pre = get_events_diff_from_coords(
        [path[0] for path in event_coords],
        profile,
        lexsort_diffs=lexsort_diffs,
        filter_missed_lohs=filter_missed_lohs,
        fail_if_empty=fail_if_empty,
    )
    diffs_post = get_events_diff_from_coords(
        [path[1] for path in event_coords],
        profile,
        lexsort_diffs=lexsort_diffs,
        filter_missed_lohs=filter_missed_lohs,
        fail_if_empty=fail_if_empty,
    )
    assert filter_missed_lohs or len(diffs_pre) == len(diffs_post), (
        len(diffs_pre),
        len(diffs_post),
    )
...
def loh_filters_for_graph_result_diffs(
    diffs,
    profile,
    ...
    if len(diffs) == 0:
        logger.warning(
            "Empty diffs passed into loh_filters_for_graph_result_diffs. Returning empty list."
        )
        return []
    ...
    log_debug(
        logger,
        f"Performing LOH filters (returning {'all solutions' if return_all_solutions else 'a single solution'})",
    )
    ...
    for n, cur_diff in enumerate(diffs):
        ...
        if shuffle_diffs:
            cur_diff = cur_diff[
                np.random.choice(np.arange(len(cur_diff)), len(cur_diff), replace=False)
            ]

        model = cp_model.CpModel()
        n_events = len(cur_diff)
        order = [
            model.NewIntVar(0, n_events - 1, f"event_{i}") for i in range(n_events)
        ]
        ...
        for diff_col, is_loh in zip(cur_diff.T, lohs):
            ...
            if (
                is_loh
                and (
                    ((diff_col == 1).sum() + (1 if total_cn else 0))
                    < (diff_col == -1).sum()
                )
                or (not is_loh and ((diff_col == -1).sum() < (2 if total_cn else 1)))
            ):
                continue
            ...
            if is_loh:
                ...
                cur_or = []
                n = 0
                for fixed_loss_i, fixed_loss_is in enumerate(
                    itertools.combinations(range(n_losses), n_losses_early)
                ):
                    fixed_losses = [losses[i] for i in fixed_loss_is]
                    free_losses = [l for l in losses if l not in fixed_losses]
                    for fixed_gains_i in itertools.combinations(
                        range(n_gains), n_gains_late
                    ):
                        fixed_gains = [gains[i] for i in fixed_gains_i]
                        free_gains_base = [
                            gains[i] for i in range(n_gains) if i not in fixed_gains_i
                        ]
                        for free_gains in itertools.permutations(free_gains_base):
                            cur_bool_or = model.NewBoolVar(
                                "loh_bool:"
                                + str(fixed_loss_i)
                                + ":"
                                + "-".join([str(i) for i in free_gains])
                            )
                            ...
                            for i in range(n_gains_late):
                                ...
                                for fixed_loss in fixed_losses:
                                    model.Add(
                                        order[fixed_loss] < order[fixed_gains[i]]
                                    ).OnlyEnforceIf(cur_bool_early_loss)
                            ...
                            for i in range(n_losses_gains_free):
                                ...
                                cur_bool_before_after = model.NewBoolVar(
                                    "loh_bool_before_after:{n}"
                                )
                                n += 1
                                ...
                                for fixed_loss in fixed_losses:
                                    model.Add(
                                        order[free_gains[i]] > order[fixed_loss]
                                    ).OnlyEnforceIf(
                                        [cur_bool_pair, cur_bool_before_after]
                                    )
                                    model.Add(
                                        order[free_gains[i]] < order[fixed_loss]
                                    ).OnlyEnforceIf(
                                        [cur_bool_pair, cur_bool_before_after.Not()]
                                    )
                                    model.Add(
                                        order[free_losses[i]] < order[fixed_loss]
                                    ).OnlyEnforceIf(
                                        [cur_bool_pair, cur_bool_before_after.Not()]
                                    )
            ...
            # in case of total_cn, one extra loss can actually be free
                ...
                for cur_losses in itertools.permutations(losses):
                    for cur_gains in itertools.combinations(gains, n_gains_early):
                        cur_bool_or = model.NewBoolVar(
                            "non-loh_bool:"
                            + "-".join([str(i) for i in cur_losses])
                            + ":"
                            + "-".join([str(i) for i in cur_gains])
                        )
                        # logger.debug(cur_bool_or) # should be commented out to save time
                        cur_and = []
                        for i in range(n_gains_early):
                            ...
                            model.Add(
                                order[cur_losses[i]] > order[cur_gains[i]]
                            ).OnlyEnforceIf(cur_bool_early_gain)
        ...
        for cur_solution_ in cp_solutions:
            ...
            assert (
                not (profile == 0).any()
                or (
                    np.cumsum(cur_diff[cur_solution], axis=0)[:, profile == 0]
            ...
            loh_happend_mask[:, profile == 0] = np.roll(
                np.pad(
                    np.cumsum(
                        np.cumsum(cur_diff[cur_solution], axis=0)[:, profile == 0]
                        == (-2 if total_cn else -1),
        ...
        log_debug(
            logger,
            f"for solution {n} found {solver_solutions.solution_count()} loh-compatible solutions of which {len(unique_results)} were unique",
        )
...
def loh_filters_for_graph_result_diffs_wgd(
    diffs, profile, return_all_solutions=True, total_cn=False, shuffle_diffs=True
):
    log_debug(
        logger,
        f"Performing LOH filters (returning {'all solutions' if return_all_solutions else 'a single solution'})",
    ...
    for n, cur_diff in enumerate(diffs):
        if shuffle_diffs:
            cur_diff = [
                cur_diff[0][
                    np.random.choice(
                        np.arange(len(cur_diff[0])), len(cur_diff[0]), replace=False
                    )
                ],
                cur_diff[1][
                    np.random.choice(
                        np.arange(len(cur_diff[1])), len(cur_diff[1]), replace=False
                    )
                ],
            ]
        ...
        order_post = [
            model.NewIntVar(0, n_post - 1, f"event_{i}") for i in range(n_post)
        ]
        if n_pre > 0:
        ...
        for diff_col_pre, diff_col_post, profile_value, post_wgd_value in zip(
            cur_diff[0].T, cur_diff[1].T, profile, post_wgd_state
        ):
            ...
            for i in range(n_pre):
                ...
                if is_loh:
                    ...
                    model.Add(
                        sum(target_elements_pre) == (-2 if total_cn else -1)
                    ).OnlyEnforceIf(current_loh_fulfilled)
                    model.Add(
                        sum(target_elements_pre) != (-2 if total_cn else -1)
                    ).OnlyEnforceIf(current_loh_fulfilled.Not())
            ...
            for i in range(n_post):
                ...
                if is_loh:
                    current_loh_fulfilled = model.NewBoolVar(f"loh_fulfilled_{i}")
                    model.Add(
                        post_wgd_value + sum(target_elements_post) == 0
                    ).OnlyEnforceIf(current_loh_fulfilled)
                    model.Add(
                        post_wgd_value + sum(target_elements_post) != 0
                    ).OnlyEnforceIf(current_loh_fulfilled.Not())
        ...
        for cur_solution in cp_solutions:
            ...
            post_wgd_state = 2 * (
                (2 if total_cn else 1) + np.sum(cur_diff_pre_solved, axis=0)
            )
            ...
            loh_happend_mask_pre[:, profile == 0] = np.roll(
                np.pad(
                    np.cumsum(
                        np.cumsum(cur_diff_pre_solved, axis=0)[:, profile == 0]
                        == (-2 if total_cn else -1),
                        axis=0,
                    ).astype(bool),
                    ((0, 1), (0, 0)),
            ...
            loh_happend_mask_post[:, profile == 0] = np.roll(
                np.pad(
                    np.cumsum(
                        post_wgd_state[profile == 0]
                        + np.cumsum(cur_diff_post_solved, axis=0)[:, profile == 0]
                        <= 0,
                        axis=0,
                    ).astype(bool),
                    ((0, 1), (0, 0)),
            ...
            cur_hash = (
                tuple(sorted(["".join(map(str, x)) for x in cur_diff_mod_pre])),
...
def check_loh_for_full_paths(full_path, solutions=None):
    ...
    if full_path.is_wgd:
        ...
        cur_diffs = [
            [
                [
                    np.fromiter(full_path.events[event].diff, dtype=int)
                    * (1 if full_path.events[event].is_gain else -1)
                    for event, count in sol.items()
                    for _ in range(count)
                    if full_path.events[event].wgd == "pre"
                ],
                [
                    np.fromiter(full_path.events[event].diff, dtype=int)
                    * (1 if full_path.events[event].is_gain else -1)
                    for event, count in sol.items()
                    for _ in range(count)
                    if full_path.events[event].wgd == "post"
                ],
            ]
            for sol_i, sol in enumerate(full_path.solutions)
            if solutions is None or sol_i in solutions
        ]
        cur_diffs = [
            [
                np.stack(cur_diff_pair[0])
                if len(cur_diff_pair[0]) > 0
                else np.empty((0, len(full_path.cn_profile)), dtype=np.int64),
                np.stack(cur_diff_pair[1])
                if len(cur_diff_pair[1]) > 0
                else np.empty((0, len(full_path.cn_profile)), dtype=np.int64),
            ]
        ...
        # log_debug(logger, cur_diffs)
        filtered_diffs = loh_filters_for_graph_result_diffs(
            cur_diffs, full_path.cn_profile, return_all_solutions=False
        )
...
def diff_to_start_end(events_df):
    ...
    for chain_nr in events_df["chain_nr"].unique():
        ...
        events[cur_events["type"].values == "loss"] = events[
            cur_events["type"].values == "loss"
        ][:, ::-1]
        repeats = np.min(
            np.stack(
                [
...
def check_pre_post_wgd_loh_viability_simple(
    which,
    loh_pos,
    pre_wgd_diffs_sum=None,
    pre_wgd_diff=None,
    post_wgd_diffs=None,
    post_wgd_state=None,
    total_cn=False,
    ...
    if which == "pre":
        ...
        loh_valid = [
            all(diff_sum[~loh_pos] >= (-1 if total_cn else 0))
            for diff_sum in pre_wgd_diffs_sum
        ]
    elif which == "post":
        remaining_loh_pos = loh_pos * (pre_wgd_diff != -1).all(axis=0)
        total_diffs = [
            (post_wgd_state + diff) for diff in post_wgd_diffs
        ]  # note that post_wgd_state already takes care of total_cn
        total_diffs_sum = [diff.sum(axis=0) for diff in total_diffs]
        # has to be >= 2 because post-WGD state can either be 0 or >=2 but not 1.
        loh_valid = [
            all(total_diff_sum[~loh_pos] >= 0)
            and (
                (post_diff[:, remaining_loh_pos] == -1).sum(axis=0)
                >= (4 if total_cn else 2)
            ).all()
            if remaining_loh_pos.any()
            else True
...
def get_events_for_cur_start_ends_wgd(
    starts, ends, n_events, cn_profile, total_cn=False, use_cache=True
):
    ...
    logger.debug(
        f"Current starts and ends (after post-WGD SV events are removed) are: {starts} / {ends}"
    )
    ...
    logger.debug(
        f"Requiring {n_required_pre_wgd_events} pre-WGD events (dist = {n_events} and {len(starts)} starts/ends and {n_post_wgd_events} post-WGD events)"
    )
    ...
    for cur_pre_start, cur_pre_end in itertools.product(
        set(
            itertools.combinations(
                pre_wgd_starts.elements(), r=n_required_pre_wgd_events
            )
        ),
        set(
            itertools.combinations(pre_wgd_ends.elements(), r=n_required_pre_wgd_events)
        ),
    ):
        ...
        if len(pre_wgd_paths) == 0:  # all events are post-WGD
            ...
            pre_wgd_diffs = get_events_diff_from_coords(
                pre_wgd_paths, cn_profile, filter_missed_lohs=False, fail_if_empty=False
            )
            pre_wgd_diffs_sum = [diff.sum(axis=0) for diff in pre_wgd_diffs]
            post_wgd_states = [
                2 * (diff.sum(axis=0) + (2 if total_cn else 1))
                for diff in pre_wgd_diffs
            ]
        ...
        for pre_wgd_path, pre_wgd_diff, post_wgd_state, valid in zip(
            pre_wgd_paths, pre_wgd_diffs, post_wgd_states, pre_wgd_loh_valid
        ):
            if not valid:
                continue
            ...
            if len(pre_wgd_path) > 0:
                [
                    cur_post_ends.remove(event[1])
                    for event in pre_wgd_path
                    for _ in range(2)
                ]
                cur_post_starts = list(
                    (
                        starts_counter
                        - Counter(2 * [event[0] for event in pre_wgd_path])
                    ).elements()
                )

            post_wgd_paths = get_events_from_graph_step(
                cur_post_starts, cur_post_ends, use_cache=use_cache
            )
            # this can happen if a start can only connect to an end that has the same value
            post_wgd_paths = [
                path for path in post_wgd_paths if len(path) == n_post_wgd_events
            ]
            ...
            if len(post_wgd_paths) == 0:
                ...
                post_wgd_diffs = get_events_diff_from_coords(
                    post_wgd_paths,
                    cn_profile,
                    lexsort_diffs=True,
                    filter_missed_lohs=False,
            ...
            post_wgd_valid = check_pre_post_wgd_loh_viability_simple(
                "post",
                loh_pos,
                pre_wgd_diff=pre_wgd_diff,
                post_wgd_diffs=post_wgd_diffs,
                post_wgd_state=post_wgd_state,
                total_cn=total_cn,
            )
            ...
            paths.append(
                [
                    (pre_wgd_path, path)
                    for path, valid in zip(post_wgd_paths, post_wgd_valid)
                    if valid
                ]
            )
...
def get_wgd_bps_to_add(paths, starts, ends, loh=True):
    ...
    min_required_pre_wgd_starts_ends = [
        reduce(Counter.__and__, all_start_end, default)
        for all_start_end, default in zip(
            list(
                zip(
                    *[
                        [
                            Counter(2 * path_start_end)
                            for path_start_end in list(
                                zip(*path[0] if len(path[0]) > 0 else [])
                            )
                        ]
                        for path in paths
                    ]
            ...
            [starts_counter, ends_counter],
        )
    ]
    min_required_pre_wgd_starts, min_required_pre_wgd_ends = (
        min_required_pre_wgd_starts_ends
        if len(min_required_pre_wgd_starts_ends) == 2
    ...
    if loh:
        for a, b in zip(
            (available_starts, available_ends), (available_ends, available_starts)
        ):
            doubled_parter_bps = np.array([k for k, v in a.items() if v >= 2])
            potential_ks = np.array(
                [k for k, v in b.items() if v % 2 == 1]
            )  # odd numbered
            bps_to_add |= set(
                potential_ks[[any(k != doubled_parter_bps) for k in potential_ks]]
            )
    else:
        for a, b, which in zip(
            (available_starts, available_ends),
            (available_ends, available_starts),
            ("larger", "smaller"),
        ):
            doubled_parter_bps = np.array([k for k, v in a.items() if v >= 2])
            potential_ks = np.array(
            ...
            bps_to_add |= set(
                potential_ks[
                    [
                        any(
                            k > doubled_parter_bps
                            if which == "larger"
                            else k < doubled_parter_bps
                        )
...
def get_new_wgd_paths_by_adding_bps(
    starts, ends, cn_profile, n_events, bps_to_add, use_cache=True, total_cn=False
):
    new_paths = []
    i = 0
    while i < len(bps_to_add):
        ...
        cur_bps_to_add = get_wgd_bps_to_add(
            cur_paths, cur_starts, cur_ends, loh=(0 in cn_profile)
        )
        ...
        log_debug(
            logger,
            f"for added bp(s) {cur_bp} found {len(cur_paths)} new paths and {len(cur_bps_to_add)} potential bps to add ({cur_bps_to_add}) of which {len(bps_to_add) - old_len_bps} were new",
        )
...
def get_sv_selected_paths_wgd(
    paths, cur_sv_data, cur_chrom_segments, sv_matching_threshold=sv_matching_threshold
):

    all_starts = np.unique(
        [
            event[0]
            for path in paths
            for pre_post in path
            for event in pre_post
            if len(pre_post) > 0
        ]
    )
    all_ends = np.unique(
    ...
    sv_overlap = np.array(
        [
            [x in path_flat for x in sv_selected_events_tuples]
            for path_flat in paths_flat
        ]
    )
    ...
    sv_selected_events = [
        event
        for event, valid in zip(sv_selected_events, sv_overlap.any(axis=0))
        if valid
    ]
...
def get_wgd_single_solution(cn_profile, max_n_iterations=25, total_cn=False):
            ...
            int(
                float(
                    fstlib.score(
                        T_forced_WGD,
                        diploid_fsa_total_cn if total_cn else diploid_fsa,
                        fsa_from_string("".join(cn_profile_.astype(str))),
                    )
                )
            )
            - 1,
    ...
    if len(early_loh_events) > 0:
        logger.debug(
            f"Found simple LOH events. Adjusted profile: {cn_profile}. Added {len(early_loh_events)} early LOH events: {early_loh_events}"
        )
    ...
    all_starts, all_ends = get_starts_and_ends_for_wgd(
        cn_profile, adjust_loh_wgd=True, total_cn=total_cn
    )
    ...
    for iteration_loh_adjust, (starts, ends) in enumerate(zip(all_starts, all_ends)):
        ...
        base_paths = __get_events_for_cur_start_ends_wgd_single_solution(
            ...
            n_events,
            cn_profile,
            select_first_solution=select_first_solution,
            total_cn=total_cn,
            max_n_iterations=max_n_iterations,
        )
        if base_paths is not None and len(base_paths) == 2:
            assert len(base_paths[0]) + len(base_paths[1]) == n_events, (
                len(base_paths[0]) + len(base_paths[1]),
                n_events,
            )
...
def _check_loh_single_solution(
    solution,
    cn_profile,
    time_limit_loh_filters=60,
    ...
    diffs = get_events_diff_from_coords_wgd(
        [solution], cn_profile, lexsort_diffs=True, filter_missed_lohs=False
    )
    if 0 in cn_profile:
        diffs = loh_filters_for_graph_result_diffs_wgd(
            diffs,
...
def __get_events_for_cur_start_ends_wgd_single_solution(
    ...
    paths = []
    # combinations is used here because there might be more potential starts/ends than required events
    for iteration_pre_selection, (cur_pre_start, cur_pre_end) in enumerate(
        itertools.product(
            set(
                itertools.combinations(
                    pre_wgd_starts.elements(), r=n_required_pre_wgd_events
            ...
            set(
                itertools.combinations(
                    pre_wgd_ends.elements(), r=n_required_pre_wgd_events
                )
            ),
        )
    ):
        # if iteration_pre_selection > max_n_iterations*10:
        if iteration_pre_selection > (
            max_n_iterations**2 if select_first_solution else max_n_iterations * 10
        ):
            break

        log_debug(
            logger, f"Current pre-WGD start and end: {cur_pre_start} / {cur_pre_end}"
        )
        if len(cur_pre_start) > 0:
            if select_first_solution:
                ...
                pre_wgd_paths = set()
                for iteration_pre_events, cur_ends in enumerate(
                    itertools.permutations(cur_pre_end)
                ):
                    if iteration_pre_events > max_n_iterations:
                        break
                    cur_pre_wgd_path = tuple(
                        sorted([(s, e) for s, e in zip(cur_pre_start, cur_ends)])
                    )
            ...
            pre_wgd_paths = [[]]
        pre_wgd_paths = [
            list(path) for path in pre_wgd_paths if len(path) == len(cur_pre_start)
        ]
        ...
        for pre_wgd_path, pre_wgd_diff, post_wgd_state, valid in zip(
            ...
            if select_first_solution:
                cur_solution = [
                    pre_wgd_path,
                    [(s, e) for s, e in zip(cur_post_starts, cur_post_ends)],
                ]
                return cur_solution
            else:
                post_wgd_paths = set()
                for iteration_post_events, cur_ends in enumerate(
                    itertools.permutations(cur_post_ends)
                ):
                    if iteration_post_events > max_n_iterations:
                        break
                    cur_post_wgd_path = tuple(
                        sorted([(s, e) for s, e in zip(cur_post_starts, cur_ends)])
                    )
...
def create_events_df_from_single_path_solution(
    full_paths, cur_id, chrom_segments=None, create_full=True, calc_telomere_bound=False
):
    unique_events_df = pd.DataFrame(full_paths.events.values())
    final_events_df = unique_events_df.iloc[
        np.array(list(full_paths.solutions[0].elements()))
    ].copy()
    ...
    final_events_df["solved"] = (
        full_paths.solved
        if full_paths.solved is not None
        else "unambiguous"
        if full_paths.n_solutions == 1
        else ""
    )
    final_events_df["id"] = cur_id
    final_events_df[["sample", "chrom", "allele"]] = cur_id.split(":")
    if create_full:
        assert chrom_segments is not None, (
            "chrom_segments must be provided if create_full is True"
        )
        final_events_df = create_full_df_from_diff_df(
            final_events_df,
            cur_id,
            chrom_segments.query("id == @cur_id").copy(),
